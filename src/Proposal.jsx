...
// ── SQUARE CONFIG ─────────────────────────────────────────────────────────────
// Sandbox: get these from https://developer.squareup.com → Sandbox tab
// To go live: swap both values for your Production App ID and Location ID
// and change SQUARE_URL to "https://web.squarecdn.com/v1/square.js"
const SQUARE_APP_ID  = import.meta.env.VITE_SQUARE_APP_ID  || "sandbox-sq0idb-REPLACE_ME";
const SQUARE_LOC_ID  = import.meta.env.VITE_SQUARE_LOCATION_ID || "REPLACE_ME";
const SQUARE_URL     = import.meta.env.VITE_SQUARE_ENV === "production"
  ? "https://web.squarecdn.com/v1/square.js"
  : "https://sandbox.web.squarecdn.com/v1/square.js";
const SQUARE_CHARGE_URL = import.meta.env.VITE_SQUARE_ENV === "production"
...
function SquarePayment({appId, locationId, squareUrl, amount, onToken, submitting, payError, setPayError}){
    ...
    const init = async () => {
      ...
      if(!window.Square){
        await new Promise((res,rej)=>{
          ...
          s.src = squareUrl;
          s.onload = res;
          s.onerror = ()=>rej(new Error("Failed to load Square SDK"));
      ...
      try{
        ...
        card = await payments.card({
          style:{
            ".input-container":{borderColor:"#E8D8D6",borderRadius:"2px"},
            ".input-container.is-focus":{borderColor:"#444444"},
            "input":{fontFamily:"'Cormorant Garamond',serif",fontSize:"18px",color:"#444444"},
        ...
        setSqError("Unable to load payment form. Please refresh and try again.");
        console.error("Square init error:", e);
      }
    };
    init();
    return ()=>{ if(cardInstanceRef.current){ try{cardInstanceRef.current.destroy();}catch(e){} } };
  },[appId, locationId, squareUrl]);
  ...
  const handlePay = async () => {
    ...
    try{
      ...
      if(result.status==="OK"){
        ...
        const msg = result.errors?.[0]?.message || "Card details are incomplete or invalid.";
        setPayError(msg);
      }
    }catch(e){
  ...
  return(
    <div style={{border:`1px solid #E8D8D6`,borderRadius:4,padding:24,marginBottom:20,background:"#fafafa"}}>
      <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:13,color:"#888",letterSpacing:"0.15em",textTransform:"uppercase",marginBottom:16}}>Card Details</div>
      {sqError&&<div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:15,color:"#8B3A3A",marginBottom:12,padding:"10px 14px",background:"#fdf0ee",borderRadius:2,border:"1px solid #e8c8c4"}}>{sqError}</div>}
      {!sqReady&&!sqError&&<div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:16,color:"#888",padding:"20px 0",textAlign:"center"}}>Loading secure payment form…</div>}
      <div ref={cardRef} style={{minHeight:89,display:sqReady?"block":"none"}}/>
      {payError&&<div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:15,color:"#8B3A3A",marginTop:12,padding:"10px 14px",background:"#fdf0ee",borderRadius:2,border:"1px solid #e8c8c4"}}>{payError}</div>}
      <div style={{marginTop:14,padding:"10px 14px",background:"#fff",borderRadius:2,border:"1px solid #E8D8D6",fontFamily:"'Cormorant Garamond',serif",fontSize:14,color:"#888",fontWeight:300}}>
        Secure payment via Square · $150 retainer charged today · Balance due 72 hrs before session
      </div>
      <button
        onClick={handlePay}
        disabled={!sqReady||submitting}
        style={{
          width:"100%",marginTop:16,background:(!sqReady||submitting)?"#e0e0e0":"#F0DEDC",
          color:(!sqReady||submitting)?"#aaa":"#444444",border:"none",borderRadius:2,
          padding:"16px",fontFamily:"'Cormorant Garamond',serif",fontSize:18,fontWeight:500,
          letterSpacing:"0.08em",cursor:(!sqReady||submitting)?"not-allowed":"pointer",
          transition:"background .18s",
        }}
      >{submitting?"Processing…":"Complete Booking — $150 Today"}</button>
      <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:14,color:"#aaa",textAlign:"center",marginTop:10,fontWeight:300}}>
        ...
        <SquarePayment
          ...
          onToken={async(token)=>{
            ...
            try{
              ...
              // NOTE: In production this should go through a backend/serverless function
              // to keep your access token secret. For now calling directly for simplicity.
              const chargeRes = await fetch(SQUARE_CHARGE_URL, {
                method:"POST",
                headers:{
                ...
                body:JSON.stringify({
                  idempotency_key: tokenRow?.token+"_"+Date.now(),
                  amount_money:{ amount:15000, currency:"USD" },
                  source_id: token,
                  note:`Retainer — ${selPkg?.name} — ${form.first} ${form.last} — ${date}`,
                  buyer_email_address: form.email,
                })
              });
              const chargeData = await chargeRes.json();
              if(chargeData.payment?.status==="COMPLETED" || chargeData.payment?.status==="APPROVED"){
                await submit();
              } else {
                const errMsg = chargeData.errors?.[0]?.detail || "Payment failed. Please try again.";
                setPayError(errMsg);
                setSubmitting(false);
              }
            }catch(e){
              setPayError("Something went wrong. Please try again or contact Michelle.");
              setSubmitting(false);
            }
          }}
