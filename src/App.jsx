...
export default function MCRM(){
  ...
  const [aiModal,setAiModal]=useState(null); // {client, emailType}
      ...
      <div style={{display:"grid",gridTemplateColumns:"1.5fr 1fr",gap:18}}>
        <div>
          <Card sx={{padding:18,marginBottom:14}}>
            <div style={{fontSize:10,letterSpacing:"0.1em",textTransform:"uppercase",color:br,fontFamily:D,fontWeight:600,marginBottom:11}}>Session Details</div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:0,borderTop:`1px solid ${bo}`}}>
              {[["Email",c.email],["Phone",c.phone],["Date",c.date||"TBD"],["Location",c.loc],["Package",p?.name||"—"],["Type",c.type]].map(([l,v])=><div key={l}><div style={{fontSize:9,letterSpacing:"0.1em",textTransform:"uppercase",color:tl,fontFamily:D,marginBottom:1}}>{l}</div><div style={{fontSize:12,color:es,fontFamily:D}}>{v}</div></div>)}
            </div>
            {c.notes&&<div style={{marginTop:11,paddingTop:11,borderTop:`1px solid ${bo}`}}><div style={{fontSize:12,color:br,fontFamily:D,lineHeight:1.6}}>{c.notes}</div></div>}
          </Card>
          <Card sx={{padding:18,marginBottom:14}}>
            <div style={{fontSize:10,letterSpacing:"0.1em",textTransform:"uppercase",color:br,fontFamily:D,fontWeight:600,marginBottom:10}}>Financials</div>
            <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:9}}>
              {[["Price",`$${p?.price||0}`],["Paid",`$${c.paid}`],["Balance",`$${c.bal}`]].map(([l,v])=><div key={l} style={{textAlign:"center",padding:9,background:cr,borderRadius:7}}><div style={{fontSize:9,letterSpacing:"0.1em",textTransform:"uppercase",color:tl,fontFamily:D,marginBottom:1}}>{l}</div><div style={{fontFamily:D,fontSize:17,color:es}}>{v}</div></div>)}
            </div>
          </Card>
          <Card sx={{padding:18}}>
            <div style={{fontSize:10,letterSpacing:"0.1em",textTransform:"uppercase",color:br,fontFamily:D,fontWeight:600,marginBottom:9}}>🖼️ Pic-Time Gallery</div>
            <input value={c.gallery} onChange={e=>setClients(cs=>cs.map(cl=>cl.id===c.id?{...cl,gallery:e.target.value}:cl))} placeholder="Paste Pic-Time URL…" style={{width:"100%",border:`1px solid ${bo}`,borderRadius:6,padding:"7px 10px",fontSize:12,fontFamily:D,color:es,outline:"none",background:cr,marginBottom:7}}/>
        ...
        <div>
          ...
          {sun&&<Card sx={{padding:14,marginBottom:12,background:`linear-gradient(135deg,#FAF3E0,${wh})`}}>
            <div style={{fontSize:10,letterSpacing:"0.1em",textTransform:"uppercase",color:br,fontFamily:D,fontWeight:600,marginBottom:9}}>🌅 Ideal Times</div>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:7}}>
              <div><div style={{fontSize:9,color:tl,fontFamily:D}}>Sunrise</div><div style={{fontFamily:D,fontSize:17,color:es}}>{sun.rs}</div></div>
              <div><div style={{fontSize:9,color:tl,fontFamily:D}}>Sunset</div><div style={{fontFamily:D,fontSize:17,color:es}}>{sun.ss}</div></div>
            </div>
            {isB(c.loc)&&p&&<div style={{background:ml,borderRadius:7,padding:"7px 11px"}}>
              <div style={{fontSize:10,fontWeight:600,color:md,fontFamily:D,marginBottom:3}}>✨ Ideal Beach Start</div>
              <div style={{fontSize:13,color:dk,fontFamily:D}}>🌅 Morning: {m2t(sun.rm-15)}</div>
              <div style={{fontSize:13,color:dk,fontFamily:D}}>🌇 Evening: {m2t(sun.sm-p.dur)}</div>
            </div>}
          </Card>}
          <Card sx={{padding:14,marginBottom:12}}>
            <div style={{fontSize:10,letterSpacing:"0.1em",textTransform:"uppercase",color:br,fontFamily:D,fontWeight:600,marginBottom:9}}>⚡ Workflow Progress</div>
            {(()=>{const allSteps=[...LWF.steps,...SWF.steps];const ci=allSteps.findIndex(s=>s.id===selC.wf);const TC2={email:{c:bl,i:"✉️"},proposal:{c:"#6B3A7A",i:"📋"},workflow:{c:gn,i:"⚡"},status:{c:md,i:"🏷️"}};return<div style={{display:"flex",flexDirection:"column",gap:4}}>{allSteps.map((s,i)=>{const done=i<ci;const curr=i===ci;const st=TC2[s.type]||TC2.email;return<div key={s.id} style={{display:"flex",alignItems:"center",gap:7,padding:"5px 7px",borderRadius:5,background:curr?ml:done?gnl:"transparent",opacity:i>ci+2?0.4:1}}>
              ...
              <div style={{flex:1}}><div style={{fontSize:12,color:es,fontFamily:D,fontWeight:curr?700:400}}>{s.a}</div><div style={{fontSize:9,color:tl,fontFamily:D}}>{s.t}</div></div>
              <span style={{fontSize:10}}>{st.i}</span>
            </div>})}</div>})()}
            <div style={{marginTop:9,display:"flex",gap:5}}>
              <select value={selC.wf||"l1"} onChange={async e=>{const upd={...selC,wf:e.target.value};setSelC(upd);setClients(cs=>cs.map(c=>c.id===upd.id?upd:c));await sb.from("clients").update({wf:e.target.value}).eq("id",upd.id);toast("Workflow step updated! ⚡");}} style={{flex:1,border:`1px solid ${bo}`,borderRadius:5,padding:"5px 7px",fontSize:11,fontFamily:D,color:es,outline:"none",background:cr}}>
                {[...LWF.steps,...SWF.steps].map(s=><option key={s.id} value={s.id}>{s.a}</option>)}
              </select>
            </div>
          </Card>
          <Card sx={{padding:14}}>
            <div style={{fontSize:10,letterSpacing:"0.1em",textTransform:"uppercase",color:br,fontFamily:D,fontWeight:600,marginBottom:9}}>Quick Actions</div>
            <div style={{display:"flex",flexDirection:"column",gap:6}}>
              <Btn v="m" ch="✨ AI Draft Email" onClick={()=>{setAiModal(selC);setAiDraft("");setAiPurpose("inquiry_response");}} sx={{background:"#F0DEDC",borderColor:bo}}/>
  ...
  const emailsTab=()=>{
    ...
    return<div className="fi">
      <H2 action={<div style={{display:"flex",gap:7}}><Btn v="m" ch="✨ AI Draft for Client" onClick={()=>{const c=clients[0];setAiModal(c);setAiDraft("");setAiPurpose("inquiry_response");}}/><Btn v="o" ch="+ New Template" onClick={()=>{const ne={id:Date.now().toString(),name:"New Template",subject:"",trigger:"",body:""};setEmails(es=>[...es,ne]);setSelE(ne)}}/></div>}>Email Templates</H2>
    ...
    if(selP)return<div className="fi">
      ...
      <Card sx={{padding:22,maxWidth:480}}>
        ...
        <label style={{display:"flex",alignItems:"center",gap:7,cursor:"pointer",marginBottom:14}}>
          ...
          <span style={{fontSize:11,fontFamily:D,color:dk}}>Mark as Most Popular</span>
        </label>
      ...
      <Card sx={{padding:20,marginBottom:18}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:`repeat(${wf.ms.length},1fr)`,gap:14}}>
          {wf.ms.map((m,mi)=><div key={m}>
            <div style={{fontSize:10,letterSpacing:"0.1em",textTransform:"uppercase",color:br,fontFamily:D,fontWeight:600,marginBottom:7}}>{m}</div>
            {wf.steps.filter(s=>s.mi===mi).map(s=>{const st=TC2[s.type]||TC2.email;return<div key={s.id} style={{marginBottom:5}}>
              ...
              <div style={{background:st.bg,borderRadius:6,padding:"7px 10px",borderLeft:`3px solid ${st.c}`}}>
                <div style={{display:"flex",gap:6,alignItems:"flex-start"}}>
                  ...
                  <div><div style={{fontSize:9,fontWeight:700,color:st.c,fontFamily:D,textTransform:"uppercase",letterSpacing:"0.07em"}}>{s.type}</div>
                    <div style={{fontSize:12,color:es,fontFamily:D}}>{s.a}</div>
                    {s.cond&&<div style={{fontSize:9,color:br,fontFamily:D,fontStyle:"italic"}}>↳ {s.cond}</div>}
                    {s.appr&&<div style={{fontSize:9,color:go,fontFamily:D}}>⚠ Needs approval</div>}
                  </div>
                </div>
              </div>
            </div>})}
      ...
      {editWF&&<Card sx={{padding:18}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:7,marginBottom:11}}>
          {(wf.id==="lead"?wfSteps.lead:wfSteps.shoot).map((s,i)=><div key={s.id} style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr auto",gap:7,alignItems:"center",background:cr,borderRadius:6,padding:"7px 9px",border:`1px solid ${bo}`}}>
            <input value={s.a} onChange={e=>{const arr=wf.id==="lead"?[...wfSteps.lead]:[...wfSteps.shoot];arr[i]={...arr[i],a:e.target.value};setWfSteps(wf.id==="lead"?{...wfSteps,lead:arr}:{...wfSteps,shoot:arr});}} style={{border:`1px solid ${bo}`,borderRadius:4,padding:"4px 7px",fontSize:11,fontFamily:D,color:es,outline:"none",background:wh}}/>
            <input value={s.t} onChange={e=>{const arr=wf.id==="lead"?[...wfSteps.lead]:[...wfSteps.shoot];arr[i]={...arr[i],t:e.target.value};setWfSteps(wf.id==="lead"?{...wfSteps,lead:arr}:{...wfSteps,shoot:arr});}} style={{border:`1px solid ${bo}`,borderRadius:4,padding:"4px 7px",fontSize:11,fontFamily:D,color:es,outline:"none",background:wh}}/>
            <select value={s.type} onChange={e=>{const arr=wf.id==="lead"?[...wfSteps.lead]:[...wfSteps.shoot];arr[i]={...arr[i],type:e.target.value};setWfSteps(wf.id==="lead"?{...wfSteps,lead:arr}:{...wfSteps,shoot:arr});}} style={{border:`1px solid ${bo}`,borderRadius:4,padding:"4px 7px",fontSize:11,fontFamily:D,color:es,outline:"none",background:wh}}>
  ...
  const AI_PURPOSES=[
    {id:"inquiry_response",label:"Inquiry Response",desc:"Warm first reply to a new lead"},
    {id:"follow_up",label:"Follow Up",desc:"Nudge after proposal sent"},
    {id:"booking_confirm",label:"Booking Confirmation",desc:"Celebrate the booking!"},
    {id:"session_reminder",label:"Session Reminder",desc:"One week before shoot"},
    {id:"day_after",label:"Day-After Thank You",desc:"After the shoot"},
    {id:"gallery_delivery",label:"Gallery Delivery",desc:"Photos are ready!"},
    {id:"print_upsell",label:"Print Upsell",desc:"Plant the seed for wall art"},
    {id:"custom",label:"Custom / Freeform",desc:"You describe it"},
  ];
...
Make it feel personal, reference their specific details naturally. Keep it warm, brief, and on-brand for Michelle.`;
    try{
      const res=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:1000,system:systemPrompt,messages:[{role:"user",content:userPrompt}]})});
      const data=await res.json();
      const text=data.content?.find(b=>b.type==="text")?.text||"";
      setAiDraft(text);
    }catch(e){setAiDraft("Could not connect to AI. Please try again.");}
    setAiLoading(false);
  };
      ...
      <div style={{background:wh,borderRadius:3,padding:28,width:"100%",maxWidth:640,boxShadow:"0 24px 70px rgba(0,0,0,.18)",border:`1px solid ${bo}`,maxHeight:"90vh",overflowY:"auto"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18}}>
          <div>
            ...
            <div style={{fontSize:12,color:br,fontFamily:D}}>For {c.first} {c.last} · {c.status}</div>
          </div>
        ...
        <div style={{marginBottom:14}}>
          <div style={{fontSize:10,letterSpacing:"0.15em",textTransform:"uppercase",color:br,fontFamily:D,fontWeight:600,marginBottom:7}}>What kind of email?</div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:6}}>
            {AI_PURPOSES.map(ap=><button key={ap.id} onClick={()=>setAiPurpose(ap.id)} style={{textAlign:"left",padding:"8px 11px",border:`1.5px solid ${aiPurpose===ap.id?es:bo}`,borderRadius:3,background:aiPurpose===ap.id?"#fdf9f9":wh,cursor:"pointer",transition:"all .12s"}}>
              <div style={{fontFamily:D,fontSize:13,color:aiPurpose===ap.id?es:br,fontWeight:aiPurpose===ap.id?600:400}}>{ap.label}</div>
              <div style={{fontSize:10,color:tl,fontFamily:D}}>{ap.desc}</div>
            </button>)}
          </div>
        </div>
        ...
        {aiLoading&&<div style={{padding:"28px 0",textAlign:"center"}}>
          <div className="pu" style={{fontSize:13,color:br,fontFamily:D,letterSpacing:"0.1em"}}>✨ Writing your email…</div>
        </div>}

        {aiDraft&&!aiLoading&&<div>
          <div style={{fontSize:10,letterSpacing:"0.15em",textTransform:"uppercase",color:br,fontFamily:D,fontWeight:600,marginBottom:7}}>Draft</div>
          <textarea value={aiDraft} onChange={e=>setAiDraft(e.target.value)} rows={12} style={{width:"100%",border:`1px solid ${bo}`,borderRadius:2,padding:"12px 14px",fontSize:14,fontFamily:D,color:es,outline:"none",resize:"vertical",background:"#fdfaf9",lineHeight:1.7}}/>
          <div style={{display:"flex",gap:7,marginTop:9}}>
            <Btn v="m" ch="📋 Copy Email" onClick={()=>{navigator.clipboard.writeText(aiDraft);toast("Email copied to clipboard! ✓")}}/>
            <Btn v="o" ch="💾 Save as Template" onClick={()=>{const ne={id:Date.now().toString(),name:`AI Draft — ${c.first} ${AI_PURPOSES.find(x=>x.id===aiPurpose)?.label}`,subject:"",trigger:"AI Generated",body:aiDraft};setEmails(es=>[...es,ne]);toast("Saved as template! ✓");setAiModal(null);setAiDraft("");}}/>
