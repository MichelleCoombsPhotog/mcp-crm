...
Make it feel personal, reference their specific details naturally. Keep it warm, brief, and on-brand for Michelle.`;
 ...
 const fetchGcalMonth=async(yr,mo)=>{
  ...
  try{
   const timeMin=new Date(yr,mo,1).toISOString().replace("Z","");
   const timeMax=new Date(yr,mo+1,0,23,59,59).toISOString().replace("Z","");
   const data=await gcalCall("gcal_list_events",{calendarId:"primary",timeMin,timeMax,maxResults:250,timeZone:"America/New_York"});
   if(data){
    ...
    items.forEach(e=>{
     ...
     if(d.getFullYear()===yr&&d.getMonth()===mo){
      ...
      byDay[day].push({
       ...
       allDay:!!(e.start&&e.start.date&&!e.start.dateTime)||e.allDay===true,
