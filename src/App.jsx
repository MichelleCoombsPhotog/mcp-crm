...
Make it feel personal, reference their specific details naturally. Keep it warm, brief, and on-brand for Michelle.`;
  ...
  const dashboard=()=><div className="fi">
    ...
    <div style={{marginBottom:18}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:9}}>
        <div style={{fontSize:10,letterSpacing:"0.1em",textTransform:"uppercase",color:br,fontFamily:D,fontWeight:600}}>📅 Upcoming Sessions</div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:18,marginBottom:18}}>
      ...
      <div>
        <div style={{fontSize:10,letterSpacing:"0.1em",textTransform:"uppercase",color:br,fontFamily:D,fontWeight:600,marginBottom:9}}>📥 Recent Leads</div>
        <Card sx={{overflow:"hidden"}}>
          {clients.filter(c=>["Lead","Proposal Sent"].includes(c.status)).slice(0,4).map((c,i,arr)=><div key={c.id} className="rh" onClick={()=>{setSelC(c);setTab("clients");}} style={{display:"flex",alignItems:"center",gap:10,padding:"11px 14px",borderBottom:i<arr.length-1?"1px solid "+bo:"none",transition:"background .14s"}}>
  ...
  const invoices=()=>{
    ...
    if(editInv){
      ...
      const items=inv._items||[{desc:p?.name||"Session",qty:1,price:p?.price||0}];
      ...
      return<div className="fi">
        <div style={{display:"flex",gap:9,alignItems:"center",marginBottom:20}}>
          <button onClick={()=>setEditInv(null)} style={{background:"transparent",border:"none",cursor:"pointer",color:br,fontSize:17}}>←</button>
          <h2 style={{fontFamily:D,fontSize:28,color:es,fontWeight:300,flex:1}}>Invoice — {isClient?inv.first+" "+inv.last:inv.customName}</h2>
          <Tag color={balance<=0?"green":inv.paid>0?"gold":"red"} ch={balance<=0?"Paid":inv.paid>0?"Partial":"Unpaid"}/>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"1.6fr 1fr",gap:18}}>
          <Card sx={{padding:24}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:22,paddingBottom:18,borderBottom:"1px solid "+bo}}>
              <div><div style={{fontFamily:D,fontSize:22,color:es}}>The Collective</div><div style={{fontSize:13,color:br,fontFamily:D}}>Michelle Coombs Photography</div><div style={{fontSize:13,color:br,fontFamily:D}}>hello@michellecoombsphotography.com</div></div>
              <div style={{textAlign:"right"}}><div style={{fontSize:10,letterSpacing:"0.1em",textTransform:"uppercase",color:br,fontFamily:D}}>Invoice #{String(inv.id).slice(-4).padStart(4,"0")}</div><div style={{fontFamily:D,fontSize:13,color:br,marginTop:4}}>{inv.date||new Date().toLocaleDateString()}</div></div>
            </div>
            <div style={{marginBottom:18}}>
              <div style={{fontSize:10,letterSpacing:"0.1em",textTransform:"uppercase",color:br,fontFamily:D,marginBottom:8}}>Bill To</div>
              <div style={{fontFamily:D,fontSize:17,color:es}}>{isClient?inv.first+" "+inv.last:inv.customName}</div>
              <div style={{fontSize:13,color:br,fontFamily:D}}>{isClient?inv.email:inv.customEmail}</div>
              {isClient&&<div style={{fontSize:13,color:br,fontFamily:D}}>{inv.phone}</div>}
            </div>
            <div style={{marginBottom:16}}>
              <div style={{display:"grid",gridTemplateColumns:"3fr 1fr 1fr 32px",gap:6,padding:"7px 0",borderBottom:"2px solid "+bo,marginBottom:6}}>
                {["Description","Qty","Price",""].map((h,i)=><div key={i} style={{fontSize:10,letterSpacing:"0.1em",textTransform:"uppercase",color:br,fontFamily:D}}>{h}</div>)}
              </div>
              {items.map((item,idx)=><div key={idx} style={{display:"grid",gridTemplateColumns:"3fr 1fr 1fr 32px",gap:6,marginBottom:6,alignItems:"center"}}>
                <input value={item.desc} onChange={e=>{const ni=[...items];ni[idx]={...ni[idx],desc:e.target.value};setEditInv({...inv,_items:ni});}} style={{border:"1px solid "+bo,borderRadius:2,padding:"7px 10px",fontFamily:D,fontSize:14,color:es,outline:"none"}}/>
                <input type="number" value={item.qty} onChange={e=>{const ni=[...items];ni[idx]={...ni[idx],qty:e.target.value};setEditInv({...inv,_items:ni});}} style={{border:"1px solid "+bo,borderRadius:2,padding:"7px 8px",fontFamily:D,fontSize:14,color:es,outline:"none"}}/>
                <input type="number" value={item.price} onChange={e=>{const ni=[...items];ni[idx]={...ni[idx],price:e.target.value};setEditInv({...inv,_items:ni});}} style={{border:"1px solid "+bo,borderRadius:2,padding:"7px 8px",fontFamily:D,fontSize:14,color:es,outline:"none"}}/>
                <button onClick={()=>{const ni=items.filter((_,i)=>i!==idx);setEditInv({...inv,_items:ni});}} style={{background:"transparent",border:"none",cursor:"pointer",color:br,fontSize:16,fontFamily:D}}>×</button>
              </div>)}
              <button onClick={()=>setEditInv({...inv,_items:[...items,{desc:"",qty:1,price:0}]})} style={{background:"transparent",border:"1px dashed "+bo,borderRadius:2,padding:"7px 14px",fontFamily:D,fontSize:13,color:br,cursor:"pointer",marginTop:6,width:"100%"}}>+ Add Line Item</button>
            </div>
            <div style={{borderTop:"1px solid "+bo,paddingTop:12,display:"flex",flexDirection:"column",alignItems:"flex-end",gap:6}}>
              <div style={{display:"flex",gap:24,fontFamily:D,fontSize:14,color:br}}><span>Subtotal</span><span>${subtotal.toLocaleString()}</span></div>
              <div style={{display:"flex",gap:24,alignItems:"center",fontFamily:D,fontSize:14}}>
                <span style={{color:br}}>Retainer/Paid</span>
                <div style={{display:"flex",alignItems:"center",gap:4}}>
                  <span style={{color:br}}>$</span>
                  <input type="number" value={inv.paid||0} onChange={e=>setEditInv({...inv,paid:Number(e.target.value)})} style={{width:80,border:"1px solid "+bo,borderRadius:2,padding:"4px 8px",fontFamily:D,fontSize:14,color:gn,outline:"none"}}/>
                </div>
              </div>
              <div style={{display:"flex",gap:24,fontFamily:D,fontSize:18,color:balance<=0?gn:rd,fontWeight:500}}><span>Balance Due</span><span>${Math.max(0,balance).toLocaleString()}</span></div>
            </div>
            {inv.notes!==undefined&&<div style={{marginTop:14}}>
              <div style={{fontSize:10,letterSpacing:"0.1em",textTransform:"uppercase",color:br,fontFamily:D,marginBottom:6}}>Notes</div>
              <textarea value={inv.notes||""} onChange={e=>setEditInv({...inv,notes:e.target.value})} rows={2} style={{width:"100%",border:"1px solid "+bo,borderRadius:2,padding:"8px 10px",fontFamily:D,fontSize:14,color:es,outline:"none",resize:"vertical"}}/>
            </div>}
            <div style={{display:"flex",gap:8,marginTop:16,flexWrap:"wrap"}}>
              <Btn v="m" ch="Save Invoice" onClick={async()=>{
                ...
                toast("Invoice saved!");setEditInv(null);
              }}/>
              ...
              {isClient&&<Btn v="o" ch="Cancel Session" onClick={()=>{
                ...
                toast("Session cancelled. Update invoice for any refunds.");
                setEditInv(upd);
              }}/>}
              <Btn v="o" ch="Send Invoice" onClick={()=>toast("Connect Resend to send invoices ✉️")}/>
            </div>
          </Card>
          <div>
            <Card sx={{padding:14,marginBottom:11}}>
              <div style={{fontSize:10,letterSpacing:"0.1em",textTransform:"uppercase",color:br,fontFamily:D,marginBottom:9}}>Payment Summary</div>
              <div style={{fontFamily:D,fontSize:28,color:balance<=0?gn:es,fontWeight:300,marginBottom:4}}>${Math.max(0,balance).toLocaleString()}</div>
              <div style={{fontSize:13,color:br,fontFamily:D,marginBottom:12}}>Balance due</div>
              {inv.paid>0&&<div style={{background:gnl,borderRadius:2,padding:"8px 12px",border:"1px solid #c3dbc3",marginBottom:8}}>
                <div style={{fontFamily:D,fontSize:13,color:gn}}>✓ ${inv.paid} received</div>
              </div>}
              {balance>0&&<div style={{background:"#fdf9f9",borderRadius:2,padding:"8px 12px",border:"1px solid "+bo}}>
                <div style={{fontFamily:D,fontSize:13,color:br}}>Due 72 hrs before session</div>
              </div>}
            </Card>
            {isClient&&<Card sx={{padding:14}}>
              <div style={{fontSize:10,letterSpacing:"0.1em",textTransform:"uppercase",color:br,fontFamily:D,marginBottom:9}}>Session Info</div>
              <div style={{fontFamily:D,fontSize:14,color:es,marginBottom:4}}>{inv.date||"Date TBD"}</div>
              <div style={{fontSize:13,color:br,fontFamily:D,marginBottom:4}}>{inv.loc}</div>
              <div style={{fontSize:13,color:br,fontFamily:D}}>{pkgs.find(x=>x.id===inv.pkg)?.name}</div>
            </Card>}
          </div>
        </div>
      </div>;
    ...
    return<div className="fi">
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:11,marginBottom:16}}>
        {[["Total Billed",`$${(rev+out).toLocaleString()}`],["Collected",`$${rev.toLocaleString()}`],["Outstanding",`$${out.toLocaleString()}`]].map(([l,v])=><Card key={l} sx={{padding:14}}><div style={{fontSize:10,letterSpacing:"0.1em",textTransform:"uppercase",color:br,fontFamily:D,fontWeight:600,marginBottom:4}}>{l}</div><div style={{fontFamily:D,fontSize:22,color:es}}>{v}</div></Card>)}
      </div>
      <Card sx={{overflow:"hidden"}}>
        <div style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr 1fr 1fr",padding:"7px 14px",borderBottom:"1px solid "+bo}}>
          {["Client","Package / Description","Date","Total","Status"].map(h=><div key={h} style={{fontSize:10,letterSpacing:"0.1em",textTransform:"uppercase",color:br,fontFamily:D}}>{h}</div>)}
        </div>
        {allInvoices.map((inv,i)=>{
          ...
          return<div key={inv.id} className="rh" onClick={()=>setEditInv({...inv,_items:inv._items||[{desc:p?.name||"Session",qty:1,price:p?.price||0}]})} style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr 1fr 1fr",padding:"12px 14px",borderBottom:i<allInvoices.length-1?"1px solid "+bo:"none",alignItems:"center",cursor:"pointer"}}>
      ...
      {newInvModal&&<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.35)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000,backdropFilter:"blur(4px)"}}>
        <div style={{background:"#fff",borderRadius:3,padding:28,width:"100%",maxWidth:520,boxShadow:"0 20px 60px rgba(0,0,0,.15)",border:"1px solid "+bo,maxHeight:"90vh",overflowY:"auto"}}>
          ...
          <div style={{marginBottom:14}}>
            <div style={{fontSize:11,letterSpacing:"0.15em",textTransform:"uppercase",color:br,fontFamily:D,marginBottom:6}}>Link to Client (optional)</div>
          ...
          <div style={{marginBottom:12}}>
            <div style={{fontSize:11,letterSpacing:"0.15em",textTransform:"uppercase",color:br,fontFamily:D,marginBottom:6}}>Line Items</div>
            {newInv.items.map((item,idx)=><div key={idx} style={{display:"grid",gridTemplateColumns:"3fr 1fr 1fr 28px",gap:6,marginBottom:6,alignItems:"center"}}>
              <input placeholder="Description" value={item.desc} onChange={e=>{const ni=[...newInv.items];ni[idx]={...ni[idx],desc:e.target.value};setNewInv({...newInv,items:ni});}} style={{border:"1px solid "+bo,borderRadius:2,padding:"7px 10px",fontFamily:D,fontSize:14,color:es,outline:"none"}}/>
  ...
  const leads=()=>{
    ...
    return<div className="fi">
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16}}>
        <h2 style={{fontFamily:D,fontSize:32,color:es,fontWeight:300}}>Leads Pipeline</h2>
          ...
          return<div key={stage} style={{minWidth:210,flex:"0 0 210px"}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:7}}>
              {stageCl.map(c=>{
                ...
                return<Card hover key={c.id} sx={{padding:12,cursor:"pointer",borderLeft:"3px solid "+(stageColors[stage]||bo)}} onClick={()=>{setSelC(c);setTab("clients");}}>
                  <div style={{fontFamily:D,fontSize:15,color:es,marginBottom:3}}>{c.first} {c.last}</div>
                  <div style={{fontSize:11,color:br,fontFamily:D,marginBottom:2}}>{c.type} · {p?.name}</div>
                  {c.date&&<div style={{fontSize:10,color:tl,fontFamily:D,marginBottom:4}}>{new Date(c.date+"T12:00:00").toLocaleDateString("en-US",{month:"short",day:"numeric",year:"numeric"})}</div>}
                  ...
                  <select value={c.status} onClick={e=>e.stopPropagation()} onChange={async e=>{e.stopPropagation();const upd={...c,status:e.target.value};setClients(cs=>cs.map(x=>x.id===upd.id?upd:x));await sb.from("clients").update({status:e.target.value}).eq("id",c.id);toast("Status → "+e.target.value);}}
                    style={{fontSize:10,border:"1px solid "+bo,borderRadius:2,padding:"3px 6px",fontFamily:D,color:br,outline:"none",cursor:"pointer",width:"100%"}}>
  ...
  const calendar=()=>{
    ...
    return<div className="fi">
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:20}}>
        <h2 style={{fontFamily:D,fontSize:32,color:es,fontWeight:300}}>{monthName}</h2>
        <div style={{display:"flex",gap:8,alignItems:"center"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 260px",gap:18}}>
        <Card sx={{padding:16}}>
          <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:2,marginBottom:6}}>
            {["Sun","Mon","Tue","Wed","Thu","Fri","Sat"].map(d=><div key={d} style={{textAlign:"center",fontSize:10,letterSpacing:"0.1em",color:br,fontFamily:D,padding:"4px 0",textTransform:"uppercase"}}>{d}</div>)}
          </div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:2}}>
            ...
            {Array.from({length:daysInMonth},(_,i)=>i+1).map(d=>{
              ...
              return<div key={d} style={{minHeight:80,borderRadius:3,border:"1px solid "+(isToday?es:bo),background:isToday?"#fdf9f9":"#fff",padding:4,cursor:hasSession?"pointer":"default",transition:"background .1s"}}
                onClick={()=>{if(daySessions.length===1){setSelC(daySessions[0]);setTab("clients");}}}>
                <div style={{fontFamily:D,fontSize:13,color:isToday?es:br,fontWeight:isToday?600:400,marginBottom:2,width:22,height:22,borderRadius:"50%",background:isToday?es:"transparent",display:"flex",alignItems:"center",justifyContent:"center",color:isToday?"#fff":br}}>{d}</div>
                {daySessions.map((c,ci)=>{const sun=getSun(c.date),p=pkgs.find(x=>x.id===c.pkg);return<div key={c.id} style={{background:pk,borderRadius:2,padding:"2px 5px",marginBottom:2,fontSize:10,fontFamily:D,color:es,lineHeight:1.3}}>
                  <div style={{fontWeight:500}}>{c.first} {c.last}</div>
        ...
        <div>
          <div style={{fontSize:10,letterSpacing:"0.1em",textTransform:"uppercase",color:br,fontFamily:D,fontWeight:600,marginBottom:9}}>Sessions This Month</div>
  ...
  const locationsTab=()=>{
    ...
    return<div className="fi">
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:11}}>
        {locations.map(loc=><Card hover key={loc.id} sx={{padding:16}}>
      ...
      return<div className="fi">
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:18}}>
          <Card sx={{padding:20}}>
            ...
            <div style={{fontSize:11,letterSpacing:"0.15em",textTransform:"uppercase",color:br,fontFamily:D,marginBottom:10}}>Questions</div>
          ...
          <div>
            <Card sx={{padding:20,marginBottom:14}}>
              <div style={{fontSize:11,letterSpacing:"0.15em",textTransform:"uppercase",color:br,fontFamily:D,marginBottom:14}}>Preview</div>
            ...
            <Card sx={{padding:16}}>
              <div style={{fontSize:11,letterSpacing:"0.15em",textTransform:"uppercase",color:br,fontFamily:D,marginBottom:10}}>Send to Client</div>
