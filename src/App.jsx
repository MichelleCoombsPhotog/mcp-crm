...
const EMAILS_D=[
 ...
 {id:"e9",name:"Gallery + Google Review",subject:"Your gallery is here — and one small favor",trigger:"2–3 weeks after gallery delivery",body:"Hi {first_name},\n\nYour gallery is ready! I had so much fun with your family and I am so proud of these images.\n\nView your gallery: {gallery_link}\n\nIf you have a moment, a Google review means the world to a small business like mine. It takes about 30 seconds and helps other families find me:\n\nhttps://g.page/r/review\n\nThank you so much for trusting me with your family. It was truly a gift.\n\nWith love,\nMichelle\nThe Collective | Michelle Coombs Photography"},
];
const CLIENTS_D=[
...
const LWF={id:"lead",name:"Lead / Proposal Track",ms:["Day 0 — Lead Added","Day 2–6 — Follow-Ups","On Booking"],steps:[
 {id:"l1",mi:0,t:"Immediately",type:"email",a:"Auto Reply",cond:"Always"},
 {id:"l2",mi:0,t:"Immediately",type:"proposal",a:"Send Booking Proposal",cond:"Always"},
 {id:"l3",mi:1,t:"Day 2 — 36 hrs later",type:"email",a:"Inquiry Response 2",cond:"If proposal not completed"},
 {id:"l4",mi:1,t:"Day 5 — 4 days later",type:"email",a:"Magic Email",cond:"If proposal not completed"},
 {id:"l5",mi:1,t:"Day 6 — auto-close",type:"status",a:"Status → Cold Lead",cond:"If proposal not completed"},
 {id:"l6",mi:2,t:"Immediately on booking",type:"workflow",a:"Apply Booked Client Track",cond:"Always"},
]};
const SWF={id:"shoot",name:"Booked Client Track",ms:["On Booking","Before Session","After Session"],steps:[
 {id:"s1",mi:0,t:"2 min after booking",type:"email",a:"Thanks for Booking",cond:"Always"},
 {id:"s2",mi:0,t:"2 hrs after booking",type:"email",a:"What to Wear Guide",cond:"Always"},
 {id:"s3",mi:1,t:"1 week before session",type:"email",a:"Session Details + Location Guide",cond:"Always"},
 {id:"s31",mi:1,t:"72 hrs before session",type:"invoice",a:"Balance Due Reminder",cond:"If balance unpaid"},
 {id:"s32",mi:1,t:"24 hrs before session",type:"invoice",a:"Urgent Balance Reminder",cond:"If balance unpaid"},
 {id:"s33",mi:1,t:"Day of session",type:"email",a:"Day-Of Reminder with directions",cond:"Always"},
 {id:"s4",mi:2,t:"3 days after session",type:"email",a:"Day-After Thank You + Google Review",cond:"Always"},
 {id:"s5",mi:2,t:"2–3 weeks after gallery delivery",type:"email",a:"Gallery Ready + Google Review Link",cond:"Always"},
]};
const NAV=[{id:"dashboard",l:"Dashboard",i:"◈"},{id:"people",l:"People",i:""},{id:"inbox",l:"Inbox",i:"️"},{id:"calendar",l:"Calendar",i:""},{id:"invoices",l:"Invoices",i:""},{id:"workflows",l:"Workflows",i:""},{id:"contracts",l:"Contracts",i:""},{id:"emails",l:"Email Templates",i:"️"},{id:"proposals",l:"Booking Proposals",i:"◻"},{id:"packages",l:"Packages",i:""},{id:"scheduler",l:"Scheduler",i:"️"},{id:"locations",l:"Locations",i:""},{id:"questionnaires",l:"Questionnaires",i:""},{id:"galleries",l:"Galleries",i:"️"},{id:"portal",l:"Client Portal",i:""},{id:"agent",l:"Agent Settings",i:"◎"},{id:"settings",l:"Settings",i:"️"}];
const LOCS_D=[
...
export default function MCRM(){
 ...
 const [collapsed,setCollapsed]=useState({contact:false,session:false});
 ...
 const [invoiceRemindersSent,setInvoiceRemindersSent]=useState({});
 const [clientMsgDraft,setClientMsgDraft]=useState("");
 const [clientMsgSub,setClientMsgSub]=useState("Re: Your Session");
 const [clientMsgs,setClientMsgs]=useState({});
 const [clientMsgsLoading,setClientMsgsLoading]=useState({});
 const [clientMsgReply,setClientMsgReply]=useState("");
 const [clientMsgReplySending,setClientMsgReplySending]=useState(false);
...
Make it feel personal, reference their specific details naturally. Keep it warm, brief, and on-brand for Michelle.`;
 ...
 const [gcalMonthEvents,setGcalMonthEvents]=useState({});
 const [gcalMonthLoading,setGcalMonthLoading]=useState(false);

 const fetchGcalMonth=async(yr,mo)=>{
  const key=yr+"-"+mo;
  setGcalMonthLoading(true);
  try{
   const timeMin=new Date(yr,mo,1).toISOString().replace(/\.\d{3}Z$/,"");
   const timeMax=new Date(yr,mo+1,0,23,59,59).toISOString().replace(/\.\d{3}Z$/,"");
   const res=await fetch("https://api.anthropic.com/v1/messages",{
    method:"POST",
    headers:{"Content-Type":"application/json","x-api-key":import.meta.env.VITE_ANTHROPIC_KEY,"anthropic-version":"2023-06-01","anthropic-beta":"mcp-client-2025-04-04","anthropic-dangerous-direct-browser-access":"true"},
    body:JSON.stringify({
     model:"claude-haiku-4-5-20251001",
     max_tokens:4096,
     mcp_servers:[{type:"url",url:"https://gcal.mcp.claude.com/mcp",name:"gcal"}],
     system:"You are a calendar data extractor. Call gcal_list_events and return ONLY a raw JSON array of events. No markdown, no explanation. Format: [{summary,start,end,allDay,id}]",
     messages:[{role:"user",content:"Call gcal_list_events with calendarId=primary, timeMin="+timeMin+", timeMax="+timeMax+", maxResults=250, timeZone=America/New_York. Return only the JSON array of events."}]
    })
   });
   if(!res.ok){setGcalMonthLoading(false);return;}
   const d=await res.json();
   // Find the tool result block or text block with JSON
   let events=[];
   if(d.content){
    for(const block of d.content){
     if(block.type==="tool_result"||block.type==="mcp_tool_result"){
      try{const r=JSON.parse(block.content?.[0]?.text||"{}");events=r.events||r.items||r||[];}catch{}
      break;
     }
     if(block.type==="text"&&block.text){
      const t=block.text.trim().replace(/^```json\n?|^```\n?|```$/g,"").trim();
      try{const p=JSON.parse(t);events=Array.isArray(p)?p:(p.events||p.items||[]);}catch{}
     }
    }
   }
   if(events.length>0){
    const byDay={};
    events.forEach(e=>{
     const startRaw=typeof e.start==="object"?(e.start.dateTime||e.start.date):e.start;
     if(!startRaw)return;
     const d=new Date(startRaw);
     if(d.getFullYear()===yr&&d.getMonth()===mo){
      const day=d.getDate();
      if(!byDay[day])byDay[day]=[];
      const endRaw=typeof e.end==="object"?(e.end.dateTime||e.end.date):e.end;
      byDay[day].push({
       summary:e.summary||"Busy",
       start:startRaw,
       end:endRaw||"",
       allDay:e.allDay||(!startRaw.includes("T")),
       isCRM:!!(e.summary&&e.summary.startsWith("MCP:")),
       id:e.id||e.summary+startRaw,
      });
     }
    });
    setGcalMonthEvents(prev=>({...prev,[key]:byDay}));
    setGcalConnected(true);
   }
  }catch(ex){console.log("fetchGcalMonth error:",ex);}
  setGcalMonthLoading(false);
 };
 ...
 const checkDateOnGCal=async(dateStr)=>{
  ...
  const data=await gcalCall("gcal_list_events",{calendarId:"primary",timeMin:dayStart.replace("Z",""),timeMax:dayEnd.replace("Z",""),maxResults:20,timeZone:"America/New_York"});
  if(!data)return{available:true,events:[],gcalOk:false};
  const items=(data.events||data.items||[]).filter(e=>!e.summary?.startsWith("MCP:"));
 ...
 const pushToGCal=async(client,pkg)=>{
  ...
  const result=await gcalCall("gcal_create_event",{
   calendarId:"primary",
...
Keep it to 4-6 sentences. Human, warm, NOT corporate. No lists. Sign as Michelle.`;
  ...
  setInboxQueue(q=>{const updated=[reviewItem,...q];sb.from("settings").upsert({key:"inbox_queue",value:JSON.stringify(updated.slice(0,50))});return updated;});
 ...
 // ── GMAIL INBOX ──────────────────────────────────────────────────
 const GMAIL_URL="https://gmail.mcp.claude.com/mcp";

 const gmailCall=async(toolName,args)=>{
  try{
   const res=await fetch(GMAIL_URL,{method:"POST",headers:{"Content-Type":"application/json"},
    body:JSON.stringify({jsonrpc:"2.0",id:Date.now(),method:"tools/call",params:{name:toolName,arguments:args}})});
   if(!res.ok)return null;
   const d=await res.json();
   const txt=d?.result?.content?.[0]?.text;
   if(!txt)return null;
   try{return JSON.parse(txt);}catch{return txt;}
  }catch(ex){console.log("Gmail error:",ex.message);return null;}
 };

 const fetchInbox=async()=>{
  setInboxLoading(true);
  try{
   const data=await gmailCall("gmail_search_messages",{query:"is:unread OR (from:(@gmail.com OR @yahoo.com OR @hotmail.com OR @icloud.com)) newer_than:30d",maxResults:50});
   if(data){
    const msgs=data.messages||data||[];
    const clientEmails=new Set(clients.map(c=>c.email.toLowerCase()));
    const threads=[];
    for(const msg of msgs.slice(0,30)){
     const detail=await gmailCall("gmail_read_message",{messageId:msg.id||msg.threadId});
     if(!detail)continue;
     const from=(detail.from||detail.headers?.from||"").toLowerCase();
     const matched=clients.find(c=>from.includes(c.email.toLowerCase()));
     threads.push({
      id:msg.id||msg.threadId,
      threadId:msg.threadId||msg.id,
      subject:detail.subject||detail.headers?.subject||"(no subject)",
      from:detail.from||detail.headers?.from||"",
      snippet:detail.snippet||detail.body?.slice(0,120)||"",
      body:detail.body||detail.textPlain||"",
      date:detail.date||detail.headers?.date||"",
      unread:msg.labelIds?.includes("UNREAD")||true,
      clientMatch:matched||null,
     });
    }
 ...
 const sendReply=async(thread,body)=>{
  setReplySending(true);
  try{
   const resend=import.meta.env.VITE_RESEND_KEY;
   if(resend){
    const toEmail=thread.from.match(/[\w.-]+@[\w.-]+\.\w+/)?.[0]||"";
    const res=await fetch("https://api.resend.com/emails",{
     method:"POST",
     headers:{"Content-Type":"application/json","Authorization":"Bearer "+resend},
     body:JSON.stringify({
      from:"Michelle Coombs <hello@michellecoombsphotography.com>",
      to:[toEmail],
      subject:"Re: "+thread.subject.replace(/^Re: /i,""),
      text:body+"\n\nWith love,\nMichelle\nThe Collective | Michelle Coombs Photography",
      reply_to:"hello@michellecoombsphotography.com",
     })
    });
    if(res.ok){
     toast("Reply sent to "+toEmail);
     setReplyDraft("");
     setSelThread(null);
     if(thread.clientMatch){
      const logEntry={from:"michelle",body,date:new Date().toISOString(),subject:"Re: "+thread.subject};
      setClients(cs=>cs.map(c=>c.id===thread.clientMatch.id?{...c,emailLog:[...(c.emailLog||[]),logEntry]}:c));
     }
    }else toast("Send failed — check Resend key");
   }else toast("Add VITE_RESEND_KEY to Vercel to send replies");
  }catch(e){toast("Error sending: "+e.message);}
  setReplySending(false);
 };

 // ── CLIENT-SPECIFIC EMAIL THREAD ────────────────────────────────
 const fetchClientMessages=async(c)=>{
  setClientMsgsLoading(p=>({...p,[c.id]:true}));
  const normalizeSubject=s=>(s||"").replace(/^(Re|Fwd|FW|RE|FWD):\s*/gi,"").trim().toLowerCase();
  try{
   const data=await gmailCall("gmail_search_messages",{query:"from:"+c.email+" OR to:"+c.email,maxResults:50});
   const msgs=data&&(data.messages||data)||[];
   const rawMsgs=[];
   for(const msg of msgs.slice(0,30)){
    const detail=await gmailCall("gmail_read_message",{messageId:msg.id||msg.threadId});
    if(!detail)continue;
    const fromAddr=(detail.from||detail.headers?.from||"").toLowerCase();
    const isMine=fromAddr.includes("michellecoombsphotography.com")||fromAddr.includes("hello@");
    rawMsgs.push({
     id:msg.id||msg.threadId,
     threadId:msg.threadId||msg.id,
     subject:detail.subject||detail.headers?.subject||"(no subject)",
     from:detail.from||detail.headers?.from||"",
     body:detail.body||detail.textPlain||detail.snippet||"",
   ...
   // Merge local emailLog
   const localLogs=(c.emailLog||[]).map((e,idx)=>({
    id:"local_"+idx,
    threadId:normalizeSubject(e.subject),
    subject:e.subject||"Sent from CRM",
    from:e.from==="michelle"?"hello@michellecoombsphotography.com":c.email,
    body:e.body||"",
    snippet:(e.body||"").slice(0,100),
    date:e.date||"",
    direction:e.from==="michelle"?"out":"in",
    local:true,
   }));
   // Deduplicate — prefer Gmail version over local log if subject+time match
   const deduped=[...rawMsgs];
   localLogs.forEach(l=>{
    const already=rawMsgs.find(r=>normalizeSubject(r.subject)===normalizeSubject(l.subject)&&Math.abs(new Date(r.date)-new Date(l.date))<180000);
    if(!already)deduped.push(l);
   });
   // Group into conversation threads by normalized subject
   const threadMap={};
   deduped.forEach(m=>{
    const key=m.threadId&&!m.local?m.threadId:normalizeSubject(m.subject);
    if(!threadMap[key])threadMap[key]={id:key,subject:m.subject,msgs:[],latestDate:""};
    threadMap[key].msgs.push(m);
    if(!threadMap[key].latestDate||new Date(m.date)>new Date(threadMap[key].latestDate))threadMap[key].latestDate=m.date;
   });
   const grouped=Object.values(threadMap).map(t=>({...t,msgs:t.msgs.sort((a,b)=>new Date(a.date)-new Date(b.date))}));
   grouped.sort((a,b)=>new Date(b.latestDate)-new Date(a.latestDate));
   setClientMsgs(p=>({...p,[c.id]:grouped}));
  }catch(e){
   // Fallback: local log only, same grouping
   const localLogs=(c.emailLog||[]).map((e,idx)=>({
    id:"local_"+idx,
    threadId:((e.subject||"")).replace(/^(Re|Fwd|FW|RE|FWD):\s*/gi,"").trim().toLowerCase(),
    subject:e.subject||"Sent from CRM",
    from:e.from==="michelle"?"hello@michellecoombsphotography.com":c.email,
    body:e.body||"",
    snippet:(e.body||"").slice(0,100),
   ...
   const grouped=Object.values(threadMap).map(t=>({...t,msgs:t.msgs.sort((a,b)=>new Date(a.date)-new Date(b.date))})).sort((a,b)=>new Date(b.latestDate)-new Date(a.latestDate));
   setClientMsgs(p=>({...p,[c.id]:grouped}));
  }
  setClientMsgsLoading(p=>({...p,[c.id]:false}));
 };

 const sendClientReply=async(c,body,subject)=>{
  setClientMsgReplySending(true);
  const resend=import.meta.env.VITE_RESEND_KEY;
  const sig="\n\nWith love,\nMichelle\nThe Collective | Michelle Coombs Photography";
  // Tracking pixel — pings Supabase edge function on open, logs to client record
  const trackId=c.id+"_"+Date.now();
  const pixelUrl=SB_URL+"/functions/v1/track-open?id="+encodeURIComponent(trackId)+"&cid="+encodeURIComponent(c.id);
   ...
   if(resend){
    const res=await fetch("https://api.resend.com/emails",{
     ...
     body:JSON.stringify({
      from:"Michelle Coombs <hello@michellecoombsphotography.com>",
      to:[c.email],
      subject:subject||"Re: Your Photography Session",
      text:body+sig,
      html:"<div style='font-family:Georgia,serif;font-size:16px;color:#444;line-height:1.8'>"+body.replace(/\n/g,"<br>")+sig.replace(/\n/g,"<br>")+"</div>"+pixelHtml,
      reply_to:"hello@michellecoombsphotography.com",
     })
    });
    const resData=await res.json().catch(()=>({}));
    if(res.ok){
     const logEntry={from:"michelle",body,date:new Date().toISOString(),subject:subject||"Sent from CRM",trackId};
     const upd={...c,emailLog:[...(c.emailLog||[]),logEntry]};
     setSelC(upd);setClients(cs=>cs.map(x=>x.id===c.id?upd:x));
     await sb.from("clients").update({emailLog:upd.emailLog}).eq("id",c.id);
     setClientMsgReply("");
     toast("Email sent to "+c.first+"!");
     fetchClientMessages(upd);
    }else{
     const errMsg=resData.message||resData.error||resData.name||"Unknown error";
     toast("Send failed: "+errMsg);
     console.error("Resend error:",resData);
    }
   }else{
    const logEntry={from:"michelle",body,date:new Date().toISOString(),subject:subject||"Sent from CRM"};
    const upd={...c,emailLog:[...(c.emailLog||[]),logEntry]};
    setSelC(upd);setClients(cs=>cs.map(x=>x.id===c.id?upd:x));
    setClientMsgReply("");
    toast("(Demo) Email logged — add VITE_RESEND_KEY to actually send");
    fetchClientMessages(upd);
   }
  }catch(e){toast("Error: "+e.message);}
  setClientMsgReplySending(false);
 };
 const checkInvoiceReminders=()=>{
  const now=new Date();
  const alerts=[];
  clients.forEach(c=>{
   if(!c.date||c.bal<=0||["Completed","Cancelled","Closed"].includes(c.status))return;
   const sessionDate=new Date(c.date+"T12:00:00");
   const hoursUntil=(sessionDate-now)/(1000*60*60);
   const alreadySent72=invoiceRemindersSent[c.id+"_72"];
   const alreadySent24=invoiceRemindersSent[c.id+"_24"];
   if(hoursUntil<=72&&hoursUntil>24&&!alreadySent72){
    alerts.push({client:c,type:"72hr",label:"72-hour balance reminder",urgent:false});
   }
   if(hoursUntil<=24&&hoursUntil>0&&!alreadySent24){
    alerts.push({client:c,type:"24hr",label:"Urgent — session is tomorrow, balance unpaid",urgent:true});
   }
  });
  return alerts;
 };

 const sendInvoiceReminder=async(c,type)=>{
  const resend=import.meta.env.VITE_RESEND_KEY;
  const body72=`Hi ${c.first},\n\nJust a quick reminder that your session balance of $${c.bal} is due 72 hours before your session on ${c.date}.\n\nPlease reach out if you have any questions about payment.\n\nSee you soon!\nMichelle\nThe Collective | Michelle Coombs Photography`;
  const body24=`Hi ${c.first},\n\nYour session is tomorrow and there is a remaining balance of $${c.bal} due before we meet.\n\nPlease arrange payment at your earliest convenience — I cannot wait to see you!\n\nWith love,\nMichelle\nThe Collective | Michelle Coombs Photography`;
  const body=type==="72hr"?body72:body24;
  const subject=type==="72hr"?"Your session balance is due — "+c.date:"Your session is tomorrow — balance due";
  if(resend){
   try{
    await fetch("https://api.resend.com/emails",{
     method:"POST",
     headers:{"Content-Type":"application/json","Authorization":"Bearer "+resend},
     body:JSON.stringify({from:"Michelle Coombs <hello@michellecoombsphotography.com>",to:[c.email],subject,text:body,reply_to:"hello@michellecoombsphotography.com"})
    });
    setInvoiceRemindersSent(p=>({...p,[c.id+"_"+type.replace("hr","")+"hr"]:true}));
    toast("Balance reminder sent to "+c.first);
   }catch(e){toast("Reminder send failed");}
  }else{
   setInvoiceRemindersSent(p=>({...p,[c.id+"_"+type]:true}));
   toast("(Demo) Reminder logged for "+c.first+" — connect Resend to send");
  }
 };
...
Keep it SHORT (5-8 sentences max). Sound like a real person, not a business. No bullet points in the email.`;
 ...
 const agentSettingsTab=()=>{
  ...
  <Btn v="o" ch="Test Connection" onClick={async()=>{
   const data=await gcalCall("gcal_list_events",{calendarId:"primary",timeMin:new Date().toISOString().replace("Z",""),maxResults:3,timeZone:"America/New_York"});
  ...
  <div style={{background:wh,borderRadius:3,padding:32,width:"100%",maxWidth:680,maxHeight:"92vh",overflowY:"auto",boxShadow:"0 28px 80px rgba(0,0,0,.22)",border:"1px solid "+bo}}>
   ...
   {isDone&&agentDraft&&<div>
    ...
    <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
     ...
     <Btn v="o" ch="Open Client" onClick={()=>{setAgentModal(null);setSelC(lead);setTab("people");}}/>
    </div>
 ...
 const load=async()=>{
 try{
 const[{data:cl},{data:pk},{data:ad},{data:em},{data:st},{data:lc},{data:qst},{data:bpt}]=await Promise.all([
  ...
  sb.from("questionnaires").select("*"),
  sb.from("booking_proposal_types").select("*"),
 ]);
 ...
 if(pk?.length)setPkgs(pk.map(p=>({...p,desc:p.description||"",image_url:p.image_url||""})));
 ...
 if(lc?.length)setLocations(lc);
 if(qst?.length)setQuestionnaires(qst);
 if(bpt?.length)setBookingProposalTypes(bpt);
 if(st?.length){
  const cv=st.find(s=>s.key==="contract");if(cv)setContract(cv.value);
  const sr=st.find(s=>s.key==="scheduler_rules");if(sr){try{setSchedulerRules(JSON.parse(sr.value));}catch{}}
  const ag=st.find(s=>s.key==="agent_settings");if(ag){try{setAgentSettings(a=>({...a,...JSON.parse(ag.value)}));}catch{}}
  const iq=st.find(s=>s.key==="inbox_queue");if(iq){try{setInboxQueue(JSON.parse(iq.value));}catch{}}
  // Load contract templates from settings
  const cts=st.filter(s=>s.key.startsWith("contract_"));
  if(cts.length){
   const loaded=cts.map(s=>({id:s.key.replace("contract_",""),name:"Contract",type:"custom",active:true,body:s.value}));
   setContractTemplates(prev=>{
    const defaults=prev.filter(p=>!loaded.find(l=>l.id===p.id));
    return[...defaults,...loaded];
   });
  }
  // Load package images
  const pimgs=st.filter(s=>s.key.startsWith("pkgimg_"));
  if(pimgs.length){
   const imgMap={};pimgs.forEach(s=>imgMap[s.key.replace("pkgimg_","")]=s.value);
   setPkgImages(imgMap);
   setPkgs(prev=>prev.map(p=>({...p,image_url:p.image_url||imgMap[p.id]||""})));
  }
 }
 ...
 const thisWeekEnd=new Date(today.getTime()+7*24*60*60*1000).toISOString().slice(0,10);
 const sessionsThisWeek=clients.filter(c=>c.date&&c.date>=todayStr&&c.date<=thisWeekEnd&&!["Cancelled","Closed"].includes(c.status)).sort((a,b)=>a.date.localeCompare(b.date));
 const needsAttention=clients.filter(c=>{
  if(c.status==="Lead"&&c.date&&new Date(c.date+"T12:00:00")<new Date(Date.now()-5*24*60*60*1000))return true;
  if(c.status==="Proposal Sent"&&c.date&&new Date(c.date+"T12:00:00")<new Date(Date.now()-3*24*60*60*1000))return true;
  if(c.bal>0&&c.date&&new Date(c.date+"T12:00:00")<new Date(Date.now()+3*24*60*60*1000)&&new Date(c.date+"T12:00:00")>new Date())return true;
  return false;
 });
 const invoiceAlerts=checkInvoiceReminders();
 // Clients where latest email in log was from them — needs a reply from Michelle
 const needsReply=clients.filter(c=>{
  const log=c.emailLog||[];
  if(log.length===0)return false;
  const sorted=[...log].sort((a,b)=>new Date(b.date)-new Date(a.date));
  return sorted[0].from!=="michelle";
 });
 const activityFeed=[
  ...clients.filter(c=>c.status==="Proposal Sent").slice(0,2).map(c=>({type:"proposal",text:`Proposal sent to ${c.first} ${c.last}`,sub:c.pkg+" · "+c.loc,time:"Recent"})),
  ...clients.filter(c=>c.status==="Booked").slice(0,2).map(c=>({type:"booked",text:`${c.first} ${c.last} booked`,sub:c.date+" · "+c.loc,time:"Recent"})),
  ...clients.filter(c=>c.status==="Closed").slice(0,1).map(c=>({type:"closed",text:`${c.first} ${c.last} marked cold`,sub:"Lead closed — no response",time:"Recent"})),
 ].slice(0,6);
 const getCountdown=(dateStr)=>{
  const diff=Math.ceil((new Date(dateStr+"T12:00:00")-new Date())/(1000*60*60*24));
  if(diff===0)return{label:"Today",color:rd};
  if(diff===1)return{label:"Tomorrow",color:go};
  if(diff<=7)return{label:`${diff} days`,color:es};
  return{label:`${diff} days`,color:br};
 };
 const greetHour=today.getHours();
 return<div className="fi">
 <div style={{marginBottom:24}}>
  <h2 style={{fontFamily:D,fontSize:38,color:es,fontWeight:300,marginBottom:4}}>{greetHour<12?"Good morning":greetHour<17?"Good afternoon":"Good evening"}, Michelle.</h2>
  <div style={{fontFamily:D,fontSize:16,color:br,fontWeight:300}}>{today.toLocaleDateString("en-US",{weekday:"long",month:"long",day:"numeric",year:"numeric"})}</div>
 </div>

 {invoiceAlerts.length>0&&<div style={{marginBottom:20,padding:"14px 20px",background:"#fdf0ee",border:"1px solid #e8c8c4",borderRadius:3}}>
  <div style={{fontFamily:D,fontSize:15,color:rd,fontWeight:500,marginBottom:8}}>Invoice reminders needed ({invoiceAlerts.length})</div>
  {invoiceAlerts.map((r,i)=><div key={i} style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:i<invoiceAlerts.length-1?6:0}}>
   <div style={{fontFamily:D,fontSize:14,color:es}}>{r.client.first} {r.client.last} — ${r.client.bal} due · {r.label}</div>
   <div style={{display:"flex",gap:6}}>
    <Btn sm v="m" ch={"Send "+r.type} onClick={()=>sendInvoiceReminder(r.client,r.type)}/>
    <Btn sm v="o" ch="Inbox" onClick={()=>setTab("inbox")}/>
   </div>
  </div>)}
 </div>}

 <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:11,marginBottom:24}}>
 {[["Revenue Collected",`$${rev.toLocaleString()}`,""],["Outstanding",`$${out.toLocaleString()}`,out>0?rd:gn],["Sessions This Week",sessionsThisWeek.length,""],["New Leads",clients.filter(c=>c.status==="Lead").length,""]].map(([l,v,vc])=><Card key={l} sx={{padding:18}}>
  <div style={{fontSize:13,letterSpacing:"0.12em",textTransform:"uppercase",color:br,fontFamily:D,marginBottom:8}}>{l}</div>
  <div style={{fontFamily:D,fontSize:36,color:vc||es,fontWeight:300,lineHeight:1}}>{v}</div>
 </Card>)}
 </div>

 <div style={{display:"grid",gridTemplateColumns:"1.4fr 1fr",gap:18,marginBottom:18}}>
  <div>
   {needsReply.length>0&&<div style={{marginBottom:18}}>
    <div style={{fontSize:13,letterSpacing:"0.12em",textTransform:"uppercase",color:rd,fontFamily:D,marginBottom:10}}>Needs Your Reply ({needsReply.length})</div>
    {needsReply.map((c)=>{
     const log=c.emailLog||[];
     const sorted=[...log].sort((a,b)=>new Date(b.date)-new Date(a.date));
     const lastMsg=sorted[0];
     const lastMsgDate=lastMsg.date?new Date(lastMsg.date).toLocaleDateString("en-US",{month:"short",day:"numeric"}):"";
     const opens=c.emailOpens||[];
     const lastOpen=opens.length>0?opens[opens.length-1]:null;
     return<Card hover key={c.id} sx={{padding:16,marginBottom:8,cursor:"pointer",borderLeft:"3px solid "+rd}} onClick={()=>{setSelC(c);setTab("people");}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:10}}>
       <div style={{flex:1,minWidth:0}}>
        <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:3}}>
         <div style={{fontFamily:D,fontSize:17,color:es}}>{c.first} {c.last}</div>
         <div style={{fontSize:10,background:rd,color:"#fff",padding:"2px 7px",borderRadius:2,fontFamily:D,letterSpacing:"0.08em",flexShrink:0}}>REPLIED</div>
        </div>
        <div style={{fontFamily:D,fontSize:13,color:br,marginBottom:2,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{lastMsg.subject||"(no subject)"} · {lastMsgDate}</div>
        <div style={{fontFamily:D,fontSize:12,color:tl,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{(lastMsg.body||"").slice(0,65)}</div>
        {lastOpen&&<div style={{fontSize:11,color:gn,fontFamily:D,marginTop:3}}>Opened your last email {new Date(lastOpen.date).toLocaleDateString("en-US",{month:"short",day:"numeric"})}</div>}
       </div>
       <Btn sm v="m" ch="Reply" onClick={e=>{e.stopPropagation();setSelC(c);setTab("people");setClientMsgReply("Hi "+c.first+",\n\n");setClientMsgSub("Re: "+(lastMsg.subject||"Your Session").replace(/^Re: /i,""));}}/>
      </div>
     </Card>;
    })}
   </div>}
   <div style={{fontSize:13,letterSpacing:"0.12em",textTransform:"uppercase",color:br,fontFamily:D,marginBottom:10}}>Sessions This Week</div>
   {sessionsThisWeek.length===0&&<Card sx={{padding:22}}><div style={{fontFamily:D,fontSize:15,color:tl}}>No sessions scheduled this week.</div></Card>}
   {sessionsThisWeek.map(c=>{
    const sun=getSun(c.date);const p=pkgs.find(x=>x.id===c.pkg);const cd=getCountdown(c.date);
    return<Card hover key={c.id} sx={{padding:18,marginBottom:10,cursor:"pointer"}} onClick={()=>{setSelC(c);setTab("people");}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
     <div>
      <div style={{fontFamily:D,fontSize:20,color:es,marginBottom:2}}>{c.first} {c.last}</div>
      <div style={{fontSize:14,color:br,fontFamily:D}}>{c.loc} · {p?p.name:"Session"}</div>
     </div>
     <div style={{textAlign:"right"}}>
      <div style={{fontFamily:D,fontSize:16,color:cd.color,fontWeight:500,marginBottom:3}}>{cd.label}</div>
      <div style={{fontSize:12,color:br,fontFamily:D}}>{new Date(c.date+"T12:00:00").toLocaleDateString("en-US",{weekday:"short",month:"short",day:"numeric"})}</div>
     </div>
    </div>
    {isB(c.loc)&&<div style={{display:"flex",gap:16,padding:"8px 12px",background:"#fdf9f9",borderRadius:2,border:"1px solid "+bo}}>
     <div style={{fontFamily:D,fontSize:13,color:es}}>Sunrise {sun.rs}</div>
     <div style={{fontFamily:D,fontSize:13,color:br}}>|</div>
     <div style={{fontFamily:D,fontSize:13,color:es}}>Sunset {sun.ss}</div>
     {p&&<><div style={{fontFamily:D,fontSize:13,color:br}}>|</div><div style={{fontFamily:D,fontSize:13,color:gn}}>Ideal: {m2t(sun.sm-p.dur)}</div></>}
    </div>}
    {c.bal>0&&<div style={{marginTop:8,padding:"6px 10px",background:"#fdf0ee",borderRadius:2,border:"1px solid #e8c8c4"}}>
     <div style={{fontFamily:D,fontSize:13,color:rd}}>Balance due: ${c.bal} — {c.bal>0&&new Date(c.date+"T12:00:00")<new Date(Date.now()+3*24*60*60*1000)?"urgent, send reminder":"pending"}</div>
    </div>}
   ...
   <div style={{fontSize:13,letterSpacing:"0.12em",textTransform:"uppercase",color:br,fontFamily:D,marginBottom:10,marginTop:18}}>My To-Do Today</div>
   {needsAttention.length===0&&<Card sx={{padding:18}}><div style={{fontFamily:D,fontSize:15,color:tl}}>Nothing urgent today. You're all caught up.</div></Card>}
   {needsAttention.map((c,i)=>{
    const reason=c.bal>0&&c.date&&new Date(c.date+"T12:00:00")<new Date(Date.now()+3*24*60*60*1000)?"Balance due soon — send reminder":c.status==="Proposal Sent"?"Proposal sent 3+ days ago — follow up?":"Lead went quiet — follow up or close";
    return<Card hover key={c.id} sx={{padding:16,marginBottom:8,cursor:"pointer",borderLeft:"3px solid "+rd}} onClick={()=>{setSelC(c);setTab("people");}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
     <div>
      <div style={{fontFamily:D,fontSize:17,color:es,marginBottom:2}}>{c.first} {c.last}</div>
      <div style={{fontSize:13,color:rd,fontFamily:D}}>{reason}</div>
     </div>
     <Btn sm v="o" ch="Open" onClick={e=>{e.stopPropagation();setSelC(c);setTab("people");}}/>
    </div>
   </Card>;
   })}
  </div>
  ...
  <div>
   <div style={{fontSize:13,letterSpacing:"0.12em",textTransform:"uppercase",color:br,fontFamily:D,marginBottom:10}}>Agent Activity</div>
   <Card sx={{padding:18,marginBottom:18}}>
    {activityFeed.length===0&&<div style={{fontFamily:D,fontSize:14,color:tl}}>No recent agent activity.</div>}
    {activityFeed.map((item,i)=><div key={i} style={{display:"flex",gap:10,alignItems:"flex-start",padding:"10px 0",borderBottom:i<activityFeed.length-1?"1px solid "+bo:"none"}}>
     <div style={{width:8,height:8,borderRadius:"50%",background:item.type==="booked"?gn:item.type==="closed"?br:bl,flexShrink:0,marginTop:6}}/>
     <div style={{flex:1}}>
      <div style={{fontFamily:D,fontSize:15,color:es,marginBottom:1}}>{item.text}</div>
      <div style={{fontSize:12,color:br,fontFamily:D}}>{item.sub}</div>
     </div>
     <div style={{fontSize:11,color:tl,fontFamily:D,flexShrink:0}}>{item.time}</div>
    </div>)}
    <button onClick={()=>setTab("people")} style={{width:"100%",marginTop:12,background:"transparent",border:"1px solid "+bo,borderRadius:2,padding:"8px",fontFamily:D,fontSize:13,color:br,cursor:"pointer"}}>View all people</button>
   </Card>

   <div style={{fontSize:13,letterSpacing:"0.12em",textTransform:"uppercase",color:br,fontFamily:D,marginBottom:10}}>Quick Stats</div>
   <Card sx={{padding:18}}>
    {[
     ["Leads in pipeline",clients.filter(c=>c.status==="Lead").length],
     ["Proposals out",clients.filter(c=>c.status==="Proposal Sent").length],
     ["Booked sessions",clients.filter(c=>["Booked","Active"].includes(c.status)).length],
     ["Completed this year",clients.filter(c=>c.status==="Completed").length],
     ["Total collected","$"+rev.toLocaleString()],
     ["Total outstanding","$"+out.toLocaleString()],
    ].map(([l,v],i,arr)=><div key={l} style={{display:"flex",justifyContent:"space-between",padding:"9px 0",borderBottom:i<arr.length-1?"1px solid "+bo:"none",alignItems:"center"}}>
     <div style={{fontFamily:D,fontSize:15,color:br}}>{l}</div>
     <div style={{fontFamily:D,fontSize:17,color:es,fontWeight:300}}>{v}</div>
    </div>)}
   </Card>
  </div>
 </div>
 ...
 <div>
 <Card sx={{padding:0,marginBottom:16,overflow:"hidden"}}>
 <div onClick={()=>setCollapsed(x=>({...x,contact:!x.contact}))} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"14px 20px",cursor:"pointer",background:"#fdf9f9",borderBottom:collapsed.contact?"none":"1px solid "+bo}}>
 <div style={{fontSize:13,letterSpacing:"0.15em",textTransform:"uppercase",color:br,fontFamily:D}}>Contact
 {collapsed.contact&&!editC&&<span style={{fontFamily:D,fontSize:13,color:tl,fontWeight:300,marginLeft:10,textTransform:"none",letterSpacing:0}}>{c.first} {c.last} · {c.email}</span>}
 </div>
 <span style={{color:br,fontSize:14,fontFamily:D}}>{collapsed.contact?"▸":"▾"}</span>
 </div>
 {!collapsed.contact&&<div style={{padding:"4px 20px 16px"}}>
 ...
 <Card sx={{padding:0,marginBottom:16,overflow:"hidden"}}>
 <div onClick={()=>setCollapsed(x=>({...x,session:!x.session}))} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"14px 20px",cursor:"pointer",background:"#fdf9f9",borderBottom:collapsed.session?"none":"1px solid "+bo}}>
 <div style={{fontSize:13,letterSpacing:"0.15em",textTransform:"uppercase",color:br,fontFamily:D}}>Session
 {collapsed.session&&!editC&&<span style={{fontFamily:D,fontSize:13,color:tl,fontWeight:300,marginLeft:10,textTransform:"none",letterSpacing:0}}>{c.date} · {c.loc}</span>}
 </div>
 <span style={{color:br,fontSize:14,fontFamily:D}}>{collapsed.session?"▸":"▾"}</span>
 </div>
 {!collapsed.session&&<div style={{padding:"4px 20px 16px"}}>
 ...
 {(()=>{
  const threadGroups=clientMsgs[c.id]||null;
  const loading=clientMsgsLoading[c.id]||false;
  const cMsg=clientMsgReply;
  return<Card sx={{padding:0,overflow:"hidden",marginBottom:16}}>
   <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"16px 22px",borderBottom:"1px solid "+bo,background:"#fdf9f9"}}>
    <div style={{fontFamily:D,fontSize:18,color:es,fontWeight:300}}>Messages</div>
    <div style={{display:"flex",gap:8,alignItems:"center"}}>
     {loading&&<div className="pu" style={{fontSize:12,color:br,fontFamily:D}}>Loading…</div>}
     {threadGroups&&<div style={{fontSize:12,color:tl,fontFamily:D}}>{threadGroups.length} thread{threadGroups.length!==1?"s":""}</div>}
     <Btn sm v="o" ch={threadGroups?"Refresh":"Load Messages"} onClick={()=>fetchClientMessages(c)}/>
     <Btn sm v="m" ch="AI Draft" onClick={()=>{setAiModal(c);setAiDraft("");setAiPurpose("inquiry_response");}}/>
    </div>
   </div>

   <div style={{padding:"14px 22px",borderBottom:"1px solid "+bo}}>
    <input value={clientMsgSub||""} onChange={e=>setClientMsgSub(e.target.value)} placeholder="Subject — new thread"
    style={{width:"100%",border:"1px solid "+bo,borderRadius:2,padding:"8px 12px",fontSize:14,fontFamily:D,color:es,outline:"none",background:"#fff",marginBottom:8}}/>
    <textarea value={cMsg} onChange={e=>setClientMsgReply(e.target.value)} rows={4} placeholder={"Hi "+c.first+",\n\n"}
    style={{width:"100%",border:"1px solid "+bo,borderRadius:2,padding:"10px 14px",fontSize:15,fontFamily:D,color:es,outline:"none",resize:"vertical",background:"#fff",lineHeight:1.8,marginBottom:8}}/>
    <div style={{display:"flex",gap:7}}>
     <Btn sm v="m" ch={clientMsgReplySending?"Sending…":"Send Message"} onClick={()=>{if(cMsg.trim())sendClientReply(c,cMsg,clientMsgSub||"");}} sx={{opacity:clientMsgReplySending?0.7:1}}/>
     <Btn sm v="o" ch="Clear" onClick={()=>{setClientMsgReply("");setClientMsgSub("");}}/>
    </div>
   </div>

   {!threadGroups&&!loading&&<div style={{padding:"24px 22px",textAlign:"center"}}>
    <div style={{fontFamily:D,fontSize:15,color:br,marginBottom:4}}>No messages loaded yet.</div>
    <div style={{fontFamily:D,fontSize:13,color:tl}}>Click Load Messages to pull every email thread with {c.email} from Gmail, plus anything sent from this CRM. Each subject line becomes its own thread — replies nest inside.</div>
   </div>}
   {threadGroups&&threadGroups.length===0&&<div style={{padding:"24px 22px"}}>
    <div style={{fontFamily:D,fontSize:15,color:br}}>No email history found for {c.email}.</div>
   </div>}

   {(threadGroups||[]).map((thread,ti)=>{
    const isExpanded=expandedMsg===thread.id;
    const latestMsg=thread.msgs[thread.msgs.length-1];
    const firstMsg=thread.msgs[0];
    const hasReply=thread.msgs.length>1;
    const lastDate=latestMsg.date?new Date(latestMsg.date).toLocaleDateString("en-US",{month:"short",day:"numeric",year:"numeric"}):"—";
    const unread=thread.msgs.some(m=>m.direction==="in"&&!m.local);
    return<div key={thread.id} style={{borderBottom:ti<(threadGroups||[]).length-1?"1px solid "+bo:"none"}}>
     {/* Thread header row — click to expand/collapse */}
     <div onClick={()=>setExpandedMsg(isExpanded?null:thread.id)} className="rh" style={{display:"grid",gridTemplateColumns:"34px 1fr 80px 24px",gap:12,alignItems:"center",padding:"12px 22px",cursor:"pointer",background:unread&&!isExpanded?"#fdfaf8":wh}}>
      <div style={{width:34,height:34,borderRadius:"50%",background:unread?es:bo,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
       <span style={{fontSize:11,color:unread?wh:br,fontFamily:D,fontWeight:500}}>{thread.msgs.length}</span>
      </div>
      <div style={{minWidth:0}}>
       <div style={{fontFamily:D,fontSize:15,color:es,fontWeight:unread?500:300,marginBottom:2,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{thread.subject||"(no subject)"}</div>
       <div style={{fontFamily:D,fontSize:12,color:br,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{latestMsg.snippet||(latestMsg.body||"").slice(0,80)}</div>
      </div>
      <div style={{textAlign:"right",fontSize:12,color:tl,fontFamily:D,flexShrink:0}}>{lastDate}</div>
      <div style={{fontSize:11,color:br,fontFamily:D,textAlign:"center"}}>{isExpanded?"▲":"▼"}</div>
     </div>

     {/* Expanded: show each message in thread */}
     {isExpanded&&<div style={{background:"#fafafa",borderTop:"1px solid "+bo}}>
      {thread.msgs.map((msg,mi)=>{
       const isOut=msg.direction==="out";
       const msgDate=msg.date?new Date(msg.date).toLocaleString("en-US",{month:"short",day:"numeric",year:"numeric",hour:"numeric",minute:"2-digit"}):"—";
       return<div key={msg.id} style={{padding:"14px 22px 14px 22px",borderBottom:mi<thread.msgs.length-1?"1px solid "+bo:"none",background:isOut?"#fafafa":wh}}>
        <div style={{display:"grid",gridTemplateColumns:"34px 1fr",gap:12,alignItems:"flex-start"}}>
         <div style={{width:34,height:34,borderRadius:"50%",background:isOut?es:pk,display:"flex",alignItems:"center",justifyContent:"center",border:"1px solid "+(isOut?"#333":bo),flexShrink:0}}>
          <span style={{fontSize:11,color:isOut?wh:es,fontFamily:D,fontWeight:500}}>{isOut?"MC":c.first.slice(0,2).toUpperCase()}</span>
         </div>
         <div>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline",marginBottom:6}}>
           <div style={{fontFamily:D,fontSize:14,color:es,fontWeight:isOut?300:500}}>{isOut?"You (Michelle)":c.first+" "+c.last}</div>
           <div style={{display:"flex",gap:8,alignItems:"center"}}>
            {isOut&&(()=>{const opens=c.emailOpens||[];const opened=opens.find(o=>o.trackId===msg.trackId||(msg.date&&Math.abs(new Date(o.date)-new Date(msg.date))<86400000*3));return opened?<div style={{fontSize:10,color:gn,fontFamily:D,letterSpacing:"0.06em"}}>OPENED {new Date(opened.date).toLocaleDateString("en-US",{month:"short",day:"numeric"})}</div>:null;})()}
            <div style={{fontSize:11,color:tl,fontFamily:D}}>{msgDate}{msg.local&&<span style={{color:br}}> · CRM</span>}{!msg.local&&!isOut&&<span style={{color:bl}}> · Gmail</span>}</div>
           </div>
          </div>
          <div style={{background:isOut?"#fff":"#fdf9f9",border:"1px solid "+bo,borderRadius:2,padding:"12px 16px",marginBottom:msg.direction==="in"?8:0}}>
           <pre style={{fontFamily:D,fontSize:14,color:es,fontWeight:300,lineHeight:1.85,whiteSpace:"pre-wrap",margin:0}}>{msg.body||msg.snippet||"(no content)"}</pre>
          </div>
          {msg.direction==="in"&&<Btn sm v="o" ch="↩ Reply to this" onClick={()=>{setClientMsgReply("Hi "+c.first+",\n\n");setClientMsgSub("Re: "+thread.subject.replace(/^Re: /i,""));}}/>}
         </div>
        </div>
       </div>;
      })}
      <div style={{padding:"12px 22px",background:"#f5f0ef",display:"flex",gap:8,alignItems:"center"}}>
       <Btn sm v="m" ch="Reply to Thread" onClick={()=>{setClientMsgReply("Hi "+c.first+",\n\n");setClientMsgSub("Re: "+thread.subject.replace(/^Re: /i,""));}}/>
       <div style={{fontFamily:D,fontSize:12,color:br}}>{thread.msgs.length} message{thread.msgs.length!==1?"s":""} in this thread</div>
      </div>
     </div>}
    </div>;
   })}
 ...
 <Btn v="o" ch="View Invoice" onClick={()=>{const pkg=pkgs.find(x=>x.id===c.pkg);setEditInv({...c,_type:"client",_items:[{desc:(pkg?pkg.name+" — "+pkg.desc:"Session"),qty:1,price:(pkg?pkg.price:0)}]});setTab("invoices");}}/>
 <Btn v="o" ch="View Contract" onClick={()=>{setTab("contracts");}}/>
 <Btn v="o" ch="Open Inbox" onClick={async()=>{
  setTab("inbox");
  // If we already have a Gmail thread for this client, jump straight to it
  const existing=inboxThreads.find(t=>t.from.toLowerCase().includes(c.email.toLowerCase())||t.clientMatch?.id===c.id);
  if(existing){setSelThread(existing);}
  else{
   // Load inbox filtered to this client, then jump
   setInboxLoading(true);
   const data=await gmailCall("gmail_search_messages",{query:"from:"+c.email+" OR to:"+c.email,maxResults:10});
   const msgs=data&&(data.messages||data)||[];
   if(msgs.length>0){
    const detail=await gmailCall("gmail_read_message",{messageId:msgs[0].id||msgs[0].threadId});
    if(detail){
     const thread={id:msgs[0].id,threadId:msgs[0].threadId||msgs[0].id,subject:detail.subject||"(no subject)",from:detail.from||c.email,snippet:detail.snippet||"",body:detail.body||detail.textPlain||"",date:detail.date||"",unread:true,clientMatch:c};
     setInboxThreads(prev=>[thread,...prev.filter(t=>t.id!==thread.id)]);
     setSelThread(thread);
    }
   }
   setInboxLoading(false);
 ...
 {c.gallery&&c.status==="Completed"&&<Btn v="o" ch="Send Gallery + Review Request" onClick={async()=>{
  const resend=import.meta.env.VITE_RESEND_KEY;
  const galleryTpl=emails.find(e=>e.id==="e9")||emails.find(e=>e.name.includes("Gallery"));
  const body=(galleryTpl?galleryTpl.body:"Hi "+c.first+",\n\nYour gallery is ready! View it here: "+c.gallery+"\n\nA quick Google review means the world:\nhttps://g.page/r/review\n\nWith love,\nMichelle").replace("{first_name}",c.first).replace("{gallery_link}",c.gallery);
  if(resend){
   await fetch("https://api.resend.com/emails",{method:"POST",headers:{"Content-Type":"application/json","Authorization":"Bearer "+resend},body:JSON.stringify({from:"Michelle Coombs <hello@michellecoombsphotography.com>",to:[c.email],subject:galleryTpl?galleryTpl.subject:"Your gallery is here",text:body})});
   toast("Gallery + review email sent to "+c.first+"!");
  }else toast("(Demo) Would send gallery + review email — add VITE_RESEND_KEY to Vercel");
 }}/>}
 </div>
 </Card>
 ...
 const [peoplePanel,setPeoplePanel]=useState(null);
 ...
 const filt=clients.filter(c=>{
  const matchSearch=!search||`${c.first} ${c.last} ${c.email} ${c.phone}`.toLowerCase().includes(search.toLowerCase());
  const matchStatus=peopleFilter==="all"||c.status===peopleFilter;
  return matchSearch&&matchStatus;
 });
 const panel=peoplePanel&&clients.find(c=>c.id===peoplePanel.id);

 return<div className="fi" style={{display:"grid",gridTemplateColumns:panel?"1fr 400px":"1fr",gap:18,alignItems:"start"}}>
  <div>
  <H2 action={<Btn v="m" ch="+ Add Person" onClick={()=>setModal("nc")}/>}>People</H2>
  <div style={{display:"flex",gap:7,marginBottom:12,flexWrap:"wrap"}}>
  {statusFilters.map(f=><button key={f.id} onClick={()=>setPeopleFilter(f.id)} style={{padding:"6px 16px",borderRadius:2,border:"1px solid "+(peopleFilter===f.id?es:bo),background:peopleFilter===f.id?es:"#fff",color:peopleFilter===f.id?"#fff":br,fontFamily:D,fontSize:14,cursor:"pointer",transition:"all .12s"}}>{f.label}{f.id!=="all"?` (${clients.filter(c=>c.status===f.id).length})`:""}</button>)}
  </div>
  <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search by name, email or phone…" style={{width:"100%",border:`1px solid ${bo}`,borderRadius:2,padding:"9px 14px",fontSize:15,fontFamily:D,color:es,outline:"none",background:wh,marginBottom:12}}/>
  <Card sx={{overflow:"hidden"}}>
  <div style={{display:"grid",gridTemplateColumns:"2fr 1fr 1.1fr 1fr .8fr .7fr",padding:"10px 18px",borderBottom:`1px solid ${bo}`}}>
  {["Name","Type","Location","Date","Status","Balance"].map(h=><div key={h} style={{fontSize:12,letterSpacing:"0.12em",textTransform:"uppercase",color:br,fontFamily:D,fontWeight:600}}>{h}</div>)}
  </div>
  {filt.map((c,i)=><div key={c.id} className="rh" onClick={()=>setPeoplePanel(peoplePanel?.id===c.id?null:c)} style={{display:"grid",gridTemplateColumns:"2fr 1fr 1.1fr 1fr .8fr .7fr",padding:"14px 18px",borderBottom:i<filt.length-1?`1px solid ${bo}`:"none",alignItems:"center",transition:"background .14s",background:peoplePanel?.id===c.id?"#fdf9f9":"transparent"}}>
  <div><div style={{fontFamily:D,fontSize:18,color:es}}>{c.first} {c.last}</div><div style={{fontSize:12,color:tl,fontFamily:D}}>{c.email}</div></div>
  <div style={{fontSize:14,color:br,fontFamily:D}}>{c.type}</div>
  <div style={{fontSize:14,color:br,fontFamily:D}}>{c.loc}</div>
  <div style={{fontSize:14,color:br,fontFamily:D}}>{c.date||"TBD"}</div>
  <Tag color={SC(c.status)} ch={c.status}/>
  <div style={{fontFamily:D,fontSize:13,color:c.bal>0?rd:gn,fontWeight:500}}>{c.bal>0?`$${c.bal}`:c.paid>0?"Paid":""}</div>
  </div>)}
  {filt.length===0&&<div style={{padding:"24px 18px",fontFamily:D,fontSize:15,color:tl}}>No people found.</div>}
  </Card>
  </div>

  {panel&&(()=>{
  const c=panel;const p=pkgs.find(x=>x.id===c.pkg);const sun=c.date?getSun(c.date):null;
  const allWFSteps=[...LWF.steps,...SWF.steps];const ci=allWFSteps.findIndex(s=>s.id===c.wf);
  const props=clientProposals[c.id]||[];
  const activeProp=props.find(pr=>pr.status==="pending"&&new Date(pr.expires_at)>new Date());
  const completedProp=props.find(pr=>pr.status==="completed");
  return<div style={{position:"sticky",top:0}}>
  <Card sx={{padding:0,overflow:"hidden",maxHeight:"calc(100vh - 120px)",overflowY:"auto"}}>
   <div style={{padding:"20px 22px",borderBottom:"1px solid "+bo,display:"flex",justifyContent:"space-between",alignItems:"flex-start",background:"#fdf9f9"}}>
    <div>
     <div style={{fontFamily:D,fontSize:24,color:es,marginBottom:3}}>{c.first} {c.last}</div>
     <div style={{fontSize:13,color:br,fontFamily:D}}>{c.email} · {c.phone}</div>
    </div>
    <div style={{display:"flex",gap:6,alignItems:"center"}}>
     <Tag color={SC(c.status)} ch={c.status}/>
     <button onClick={()=>setPeoplePanel(null)} style={{background:"transparent",border:"none",fontSize:20,cursor:"pointer",color:br,marginLeft:4}}>×</button>
    </div>
   </div>

   <div style={{padding:"16px 22px",borderBottom:"1px solid "+bo}}>
    <div style={{fontSize:12,letterSpacing:"0.12em",textTransform:"uppercase",color:br,fontFamily:D,marginBottom:10}}>Session</div>
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:8}}>
     {[["Date",c.date||"TBD"],["Location",c.loc||"TBD"],["Package",p?p.name:"TBD"],["Type",c.type||"Family"]].map(([l,v])=><div key={l}><div style={{fontSize:11,color:tl,fontFamily:D,textTransform:"uppercase",letterSpacing:"0.1em",marginBottom:2}}>{l}</div><div style={{fontFamily:D,fontSize:15,color:es,fontWeight:300}}>{v}</div></div>)}
    </div>
    {sun&&isB(c.loc)&&<div style={{background:pk,borderRadius:2,padding:"7px 12px",border:"1px solid "+bo,fontSize:13,fontFamily:D,color:es}}>Beach: Sunrise {sun.rs} · Sunset {sun.ss}{p?` · Ideal start: ${m2t(sun.sm-p.dur)}`:""}</div>}
   </div>

   <div style={{padding:"14px 22px",borderBottom:"1px solid "+bo}}>
    <div style={{fontSize:12,letterSpacing:"0.12em",textTransform:"uppercase",color:br,fontFamily:D,marginBottom:8}}>Financials</div>
    <div style={{display:"flex",gap:12}}>
     {[["Package",`$${p?p.price:0}`],["Paid",`$${c.paid}`],["Balance",`$${c.bal}`]].map(([l,v])=><div key={l} style={{flex:1,textAlign:"center",padding:"10px 6px",background:"#fdf9f9",borderRadius:2,border:"1px solid "+bo}}><div style={{fontSize:11,color:tl,fontFamily:D,textTransform:"uppercase",marginBottom:2}}>{l}</div><div style={{fontFamily:D,fontSize:18,color:l==="Balance"&&c.bal>0?rd:es,fontWeight:300}}>{v}</div></div>)}
    </div>
   </div>

   <div style={{padding:"14px 22px",borderBottom:"1px solid "+bo}}>
    <div style={{fontSize:12,letterSpacing:"0.12em",textTransform:"uppercase",color:br,fontFamily:D,marginBottom:8}}>Workflow</div>
    <div style={{display:"flex",flexDirection:"column",gap:3,marginBottom:8}}>
    {allWFSteps.slice(Math.max(0,ci-1),ci+4).map((s,i,arr)=>{
     const actualIdx=allWFSteps.findIndex(x=>x.id===s.id);
     const done=actualIdx<ci;const curr=actualIdx===ci;
     return<div key={s.id} style={{display:"flex",alignItems:"center",gap:7,padding:"5px 8px",borderRadius:2,background:curr?pk:done?gnl:"transparent"}}>
      <div style={{width:12,height:12,borderRadius:"50%",background:curr?"#444":done?gn:bo,flexShrink:0}}/>
      <div style={{fontFamily:D,fontSize:13,color:curr?es:done?gn:br,fontWeight:curr?500:300}}>{s.a}</div>
     </div>;
    })}
    </div>
   </div>

   {c.notes&&<div style={{padding:"14px 22px",borderBottom:"1px solid "+bo}}>
    <div style={{fontSize:12,letterSpacing:"0.12em",textTransform:"uppercase",color:br,fontFamily:D,marginBottom:6}}>Notes</div>
    <div style={{fontFamily:D,fontSize:14,color:es,fontWeight:300,lineHeight:1.6}}>{c.notes}</div>
   </div>}

   <div style={{padding:"14px 22px",borderBottom:"1px solid "+bo}}>
    <div style={{fontSize:12,letterSpacing:"0.12em",textTransform:"uppercase",color:br,fontFamily:D,marginBottom:8}}>Booking Proposal</div>
    {completedProp&&<div style={{background:gnl,borderRadius:2,padding:"8px 12px",border:"1px solid #c3dbc3"}}><div style={{fontFamily:D,fontSize:14,color:gn}}>Proposal completed · Booked</div></div>}
    {activeProp&&<div><div style={{background:"#fdf9f9",borderRadius:2,padding:"8px 12px",border:"1px solid "+bo,marginBottom:8}}><div style={{fontSize:12,color:br,fontFamily:D,marginBottom:4}}>Active link</div><div style={{fontSize:12,fontFamily:D,color:es,wordBreak:"break-all"}}>{window.location.origin+"/proposal?token="+activeProp.token}</div></div><Btn sm v="m" ch="Copy Link" onClick={()=>{navigator.clipboard.writeText(window.location.origin+"/proposal?token="+activeProp.token);toast("Link copied!");}}/></div>}
    {!completedProp&&!activeProp&&<Btn sm v="o" ch="Generate Proposal Link" onClick={async()=>{const token=Math.random().toString(36).slice(2)+Math.random().toString(36).slice(2);const expiresAt=new Date(Date.now()+14*24*60*60*1000).toISOString();const row={token,client_id:c.id,client_first:c.first,client_last:c.last,client_email:c.email,status:"pending",expires_at:expiresAt,created_at:new Date().toISOString()};await sb.from("proposals").insert(row);setClientProposals(cp=>({...cp,[c.id]:[...(cp[c.id]||[]),row]}));toast("Proposal link generated!");}}/>}
   </div>

   <div style={{padding:"16px 22px"}}>
    <div style={{fontSize:12,letterSpacing:"0.12em",textTransform:"uppercase",color:br,fontFamily:D,marginBottom:10}}>Quick Actions</div>
    <div style={{display:"flex",flexDirection:"column",gap:7}}>
     <Btn v="m" ch="Open Full Profile" onClick={()=>{setSelC(c);setTab("people");}}/>
     <Btn v="o" ch="AI Draft Email" onClick={()=>{setAiModal(c);setAiDraft("");setAiPurpose("inquiry_response");}}/>
     <Btn v="o" ch="View Invoice" onClick={()=>{const pkg=pkgs.find(x=>x.id===c.pkg);setEditInv({...c,_type:"client",_items:[{desc:(pkg?pkg.name+" — "+pkg.desc:"Session"),qty:1,price:(pkg?p?p.price:0:0)}]});setTab("invoices");}}/>
     {c.status==="Completed"&&c.gallery&&<Btn v="o" ch="Send Gallery + Review Request" onClick={async()=>{const resend=import.meta.env.VITE_RESEND_KEY;const galleryTpl=emails.find(e=>e.id==="e9")||emails.find(e=>e.name.includes("Gallery"));const body=(galleryTpl?galleryTpl.body:"Hi "+c.first+",\n\nYour gallery is ready! View it here: "+c.gallery+"\n\nA Google review would mean the world:\nhttps://g.page/r/review\n\nWith love,\nMichelle").replace("{first_name}",c.first).replace("{gallery_link}",c.gallery);if(resend){await fetch("https://api.resend.com/emails",{method:"POST",headers:{"Content-Type":"application/json","Authorization":"Bearer "+resend},body:JSON.stringify({from:"Michelle Coombs <hello@michellecoombsphotography.com>",to:[c.email],subject:galleryTpl?galleryTpl.subject:"Your gallery is here",text:body})});toast("Gallery + review email sent to "+c.first+"!");}else toast("(Demo) Would send gallery + review email — connect Resend");}}/>}
     <Btn v="o" ch="Run Agent" onClick={()=>setAgentModal(c)}/>
    </div>
   </div>
  </Card>
  </div>;
 ...
 // Auto-sync: any booking proposal type using this package now reflects the updated name/price/desc
 // pkgs state update above means next render of proposal previews automatically uses latest data — no extra work needed
 // but log it so Michelle sees it happened
 const affectedBPs=bookingProposalTypes.filter(bp=>bp.pkgIds.includes(selP.id));
 if(affectedBPs.length>0)toast("Saved! Package updated in "+affectedBPs.length+" proposal type"+(affectedBPs.length>1?"s":"")+" automatically.");
 else toast("Saved!");
 setSelP(null);
 return;
 }
 ...
 const workflows=()=>{
 const wfKey=selWF?selWF.id==="lead"?"lead":"shoot":"lead";
 const activeWF=selWF||(wfKey==="lead"?LWF:SWF);
 const steps=editWF?(wfKey==="lead"?wfSteps.lead:wfSteps.shoot):activeWF.steps;

 const STEP_TYPES=[
  {id:"email",label:"Send Email",color:bl,bg:bll},
  {id:"proposal",label:"Send Proposal",color:"#6B3A7A",bg:"#F0EBF5"},
  {id:"status",label:"Change Status",color:md,bg:ml},
  {id:"workflow",label:"Apply Workflow",color:gn,bg:gnl},
  {id:"invoice",label:"Send Invoice Reminder",color:rd,bg:"#F5EBEB"},
 ];
 const getTypeStyle=(type)=>STEP_TYPES.find(t=>t.id===type)||STEP_TYPES[0];

 const COND_OPTIONS=["Always","If balance unpaid","If not yet booked","If proposal not completed","If balance paid"];
 const EMAIL_OPTIONS=["Auto Reply","Inquiry Response 2","Magic Email","Thanks for Booking","What to Wear Guide","Session Details","Day-After Thank You","Gallery Ready",...emails.map(e=>e.name)];
 const STATUS_OPTIONS=["Lead","Proposal Sent","Booked","Active","Completed","Closed","Cold Lead"];

 const updateStep=(id,field,val)=>{
  const arr=[...(wfKey==="lead"?wfSteps.lead:wfSteps.shoot)];
  const idx=arr.findIndex(s=>s.id===id);
  if(idx>-1){arr[idx]={...arr[idx],[field]:val};setWfSteps(wfKey==="lead"?{...wfSteps,lead:arr}:{...wfSteps,shoot:arr});}
 };

 const LEAD_TRACK=[
  {label:"Day 0 — Lead Added",steps:steps.filter(s=>s.mi===0)},
  {label:"Day 2–6 — Follow-Ups",steps:steps.filter(s=>s.mi===1)},
  {label:"On Booking",steps:steps.filter(s=>s.mi===2)},
 ];
 const SHOOT_TRACK=[
  {label:"On Booking",steps:steps.filter(s=>s.mi===0)},
  {label:"Before Session",steps:steps.filter(s=>s.mi===1)},
  {label:"After Session",steps:steps.filter(s=>s.mi===2)},
 ];
 const tracks=wfKey==="lead"?LEAD_TRACK:SHOOT_TRACK;

 return<div className="fi">
 <H2>Automation Workflows</H2>

 <div style={{display:"flex",gap:9,marginBottom:20}}>
 {[{id:"lead",name:"Lead / Proposal Track",desc:"From first inquiry to booked"},{id:"shoot",name:"Booked Client Track",desc:"From booking through post-session"}].map(w=><button key={w.id} onClick={()=>{setSelWF(w);setEditWF(false);}} style={{padding:"12px 20px",borderRadius:2,border:`2px solid ${(selWF?selWF.id:wfKey)===w.id?es:bo}`,background:(selWF?selWF.id:wfKey)===w.id?es:"#fff",color:(selWF?selWF.id:wfKey)===w.id?"#fff":es,fontFamily:D,fontSize:15,cursor:"pointer",textAlign:"left",transition:"all .12s"}}>
  <div style={{fontWeight:400,marginBottom:2}}>{w.name}</div>
  <div style={{fontSize:12,opacity:0.7,fontFamily:D}}>{w.desc}</div>
 </button>)}
 </div>

 <div style={{marginBottom:20,padding:"14px 20px",background:"#fdf9f9",border:"1px solid "+bo,borderRadius:3,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
  <div style={{fontFamily:D,fontSize:15,color:es}}>
  {wfKey==="lead"?"Automatically runs when a new lead is added. Stops all emails when lead books or goes cold.":"Automatically runs when a client books. Ties to their session date for precise timing."}
  </div>
  <div style={{display:"flex",gap:8,flexShrink:0,marginLeft:16}}>
  {editWF&&<Btn sm v="m" ch="Save Workflow" onClick={()=>{toast("Workflow saved!");setEditWF(false);}}/>}
  <Btn sm v="o" ch={editWF?"Cancel Edit":"Edit Workflow"} onClick={()=>setEditWF(!editWF)}/>
  {editWF&&<Btn sm v="o" ch="+ Add Step" onClick={()=>{const ns={id:Date.now().toString(),mi:0,t:"Immediately",type:"email",a:"New Email Step",cond:"Always"};setWfSteps(wfKey==="lead"?{...wfSteps,lead:[...wfSteps.lead,ns]}:{...wfSteps,shoot:[...wfSteps.shoot,ns]});}}/>}
  </div>
 </div>

 <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:14,marginBottom:24}}>
 {tracks.map((track,ti)=><div key={ti}>
  <div style={{fontSize:13,letterSpacing:"0.12em",textTransform:"uppercase",color:br,fontFamily:D,marginBottom:10,paddingBottom:8,borderBottom:"2px solid "+bo}}>{track.label}</div>
  <div style={{display:"flex",flexDirection:"column",gap:8}}>
  {track.steps.map((s)=>{
   const ts=getTypeStyle(s.type);
   if(editWF)return<div key={s.id} style={{background:wh,borderRadius:2,border:"1px solid "+bo,padding:12}}>
    <div style={{display:"flex",gap:6,marginBottom:6,alignItems:"center"}}>
     <select value={s.type} onChange={e=>updateStep(s.id,"type",e.target.value)} style={{border:"1px solid "+bo,borderRadius:2,padding:"4px 7px",fontSize:12,fontFamily:D,color:es,outline:"none",background:wh,flex:1}}>
     {STEP_TYPES.map(t=><option key={t.id} value={t.id}>{t.label}</option>)}
     </select>
     <button onClick={()=>{const arr=(wfKey==="lead"?wfSteps.lead:wfSteps.shoot).filter(x=>x.id!==s.id);setWfSteps(wfKey==="lead"?{...wfSteps,lead:arr}:{...wfSteps,shoot:arr});}} style={{background:"#fdf0ee",border:"none",borderRadius:2,padding:"4px 9px",cursor:"pointer",color:rd,fontSize:14,fontFamily:D}}>×</button>
    </div>
    {(s.type==="email"||s.type==="proposal")&&<select value={s.a} onChange={e=>updateStep(s.id,"a",e.target.value)} style={{width:"100%",border:"1px solid "+bo,borderRadius:2,padding:"5px 8px",fontSize:12,fontFamily:D,color:es,outline:"none",background:wh,marginBottom:5}}>
    {EMAIL_OPTIONS.map(o=><option key={o}>{o}</option>)}
    </select>}
    {s.type==="status"&&<select value={s.a} onChange={e=>updateStep(s.id,"a","Status → "+e.target.value)} style={{width:"100%",border:"1px solid "+bo,borderRadius:2,padding:"5px 8px",fontSize:12,fontFamily:D,color:es,outline:"none",background:wh,marginBottom:5}}>
    {STATUS_OPTIONS.map(o=><option key={o}>{o}</option>)}
    </select>}
    {s.type!=="email"&&s.type!=="proposal"&&s.type!=="status"&&<input value={s.a} onChange={e=>updateStep(s.id,"a",e.target.value)} style={{width:"100%",border:"1px solid "+bo,borderRadius:2,padding:"5px 8px",fontSize:12,fontFamily:D,color:es,outline:"none",background:wh,marginBottom:5}}/>}
    <input value={s.t} onChange={e=>updateStep(s.id,"t",e.target.value)} placeholder="Delay (e.g. Day 2, 72 hrs before session)" style={{width:"100%",border:"1px solid "+bo,borderRadius:2,padding:"5px 8px",fontSize:12,fontFamily:D,color:es,outline:"none",background:wh,marginBottom:5}}/>
    <select value={s.cond||"Always"} onChange={e=>updateStep(s.id,"cond",e.target.value)} style={{width:"100%",border:"1px solid "+bo,borderRadius:2,padding:"5px 8px",fontSize:12,fontFamily:D,color:es,outline:"none",background:wh}}>
    {COND_OPTIONS.map(o=><option key={o}>{o}</option>)}
    </select>
   </div>;
   return<div key={s.id} style={{background:ts.bg,borderRadius:2,padding:"10px 14px",border:`1px solid ${bo}`,borderLeft:`3px solid ${ts.color}`}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:3}}>
     <div style={{fontSize:11,letterSpacing:"0.1em",textTransform:"uppercase",color:ts.color,fontFamily:D,fontWeight:600}}>{STEP_TYPES.find(t=>t.id===s.type)?.label||s.type}</div>
     {s.cond&&s.cond!=="Always"&&<div style={{fontSize:10,color:br,fontFamily:D,background:wh,padding:"2px 7px",borderRadius:2,border:"1px solid "+bo}}>{s.cond}</div>}
    </div>
    <div style={{fontFamily:D,fontSize:14,color:es,fontWeight:400,marginBottom:2}}>{s.a}</div>
    <div style={{fontSize:11,color:br,fontFamily:D}}>{s.t}</div>
   </div>;
  })}
  {editWF&&<button onClick={()=>{const ns={id:Date.now().toString(),mi:ti,t:"Immediately",type:"email",a:"New Step",cond:"Always"};setWfSteps(wfKey==="lead"?{...wfSteps,lead:[...wfSteps.lead,ns]}:{...wfSteps,shoot:[...wfSteps.shoot,ns]});}} style={{width:"100%",background:"transparent",border:`1px dashed ${bo}`,borderRadius:2,padding:"8px",fontFamily:D,fontSize:13,color:br,cursor:"pointer",marginTop:4}}>+ Add Step</button>}
  </div>
 </div>)}
 </div>

 <H2>Active in Workflow</H2>
 <Card sx={{overflow:"hidden",marginBottom:14}}>
 {clients.filter(c=>!["Completed","Closed"].includes(c.status)).map((c,i,arr)=>{
  const allSteps=[...LWF.steps,...SWF.steps];const cs=allSteps.find(s=>s.id===c.wf);
  return<div key={c.id} className="rh" onClick={()=>{setSelC(c);setTab("people");}} style={{display:"flex",alignItems:"center",gap:11,padding:"14px 18px",borderBottom:i<arr.length-1?`1px solid ${bo}`:"none",cursor:"pointer"}}>
  <div style={{width:7,height:7,borderRadius:"50%",background:gn,flexShrink:0}} className="pu"/>
  <div style={{flex:1}}><div style={{fontFamily:D,fontSize:18,color:es}}>{c.first} {c.last}</div><div style={{fontSize:12,color:br,fontFamily:D}}>{cs?.a||"Active"} · {cs?.t||""}</div></div>
  <Tag color={SC(c.status)} ch={c.status}/>
  <div style={{fontSize:13,color:c.bal>0?rd:gn,fontFamily:D}}>{c.bal>0?"$"+c.bal+" due":""}</div>
  </div>;
 })}
 {clients.filter(c=>!["Completed","Closed"].includes(c.status)).length===0&&<div style={{padding:"22px 18px",fontFamily:D,fontSize:15,color:tl}}>No active clients in workflow.</div>}
 </Card>
 </div>;
 };
 ...
 if(selContract){
  ...
  {clientsWithThisContract.map(cl=><div key={cl.id} className="rh" onClick={()=>{setSelC(cl);setTab("people");}} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 0",borderBottom:"1px solid "+bo,cursor:"pointer"}}>
 ...
 {(()=>{
  ...
  const dueThisWeek=allInvoices.filter(inv=>{
   ...
   return inv.date>=todayStr&&inv.date<=endOfWeekStr;
  });
  const dueThisMonth=allInvoices.filter(inv=>{
   ...
   if(bal<=0)return false;
   if(!inv.date)return false;
   return inv.date>endOfWeekStr&&inv.date<=endOfMonthStr;
  });
  if(dueThisWeek.length===0&&dueThisMonth.length===0)return null;
  const InvRow=({inv,tint})=>{
   const p=inv._type==="client"?pkgs.find(x=>x.id===inv.pkg):null;
   const items=inv._items||[{price:p?.price||0,qty:1}];
   const total=inv._type==="client"?(p?.price||0):items.reduce((s,it)=>s+it.qty*it.price,0);
   const bal=total-(inv.paid||0);
   const name=inv._type==="client"?inv.first+" "+inv.last:inv.customName;
   const dateDisp=inv.date?new Date(inv.date+"T12:00:00").toLocaleDateString("en-US",{weekday:"short",month:"short",day:"numeric"}):"—";
   return<div className="rh" onClick={()=>setEditInv({...inv,_items:inv._items||[{desc:p?.name||"Session",qty:1,price:p?.price||0}]})}
    style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr 1fr",padding:"13px 18px",borderBottom:"1px solid "+bo,alignItems:"center",cursor:"pointer",background:tint}}>
    <div><div style={{fontFamily:D,fontSize:16,color:es}}>{name}</div><div style={{fontSize:12,color:tl,fontFamily:D}}>{inv.loc||""}</div></div>
    <div style={{fontSize:13,color:br,fontFamily:D}}>{dateDisp}</div>
    <div style={{fontFamily:D,fontSize:15,color:rd,fontWeight:500}}>${Math.max(0,bal).toLocaleString()} due</div>
    <Tag color={inv.paid>0?"gold":"red"} ch={inv.paid>0?"Partial":"Unpaid"}/>
   </div>;
  };
  return<div style={{marginBottom:18}}>
   {dueThisWeek.length>0&&<div style={{marginBottom:14}}>
    <div style={{fontSize:13,letterSpacing:"0.15em",textTransform:"uppercase",color:rd,fontFamily:D,fontWeight:600,marginBottom:8,display:"flex",alignItems:"center",gap:8}}>
     Due This Week
     <span style={{background:"#f5e8e8",color:rd,fontSize:12,padding:"2px 10px",borderRadius:2,border:"1px solid #e8c8c4"}}>{dueThisWeek.length} invoice{dueThisWeek.length>1?"s":""} · ${dueThisWeek.reduce((s,inv)=>{const p=inv._type==="client"?pkgs.find(x=>x.id===inv.pkg):null;const items=inv._items||[{price:p?.price||0,qty:1}];const total=inv._type==="client"?(p?.price||0):items.reduce((ss,it)=>ss+it.qty*it.price,0);return s+Math.max(0,total-(inv.paid||0));},0).toLocaleString()} outstanding</span>
    </div>
    <Card sx={{overflow:"hidden"}}>
     <div style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr 1fr",padding:"7px 18px",borderBottom:"1px solid "+bo,background:"#fdf5f5"}}>
      {["Client","Session Date","Balance","Status"].map(h=><div key={h} style={{fontSize:12,letterSpacing:"0.1em",textTransform:"uppercase",color:br,fontFamily:D}}>{h}</div>)}
     </div>
     {dueThisWeek.map(inv=><InvRow key={inv.id} inv={inv} tint="#fffafa"/>)}
    </Card>
   </div>}
   {dueThisMonth.length>0&&<div style={{marginBottom:14}}>
    <div style={{fontSize:13,letterSpacing:"0.15em",textTransform:"uppercase",color:"#7A5A2A",fontFamily:D,fontWeight:600,marginBottom:8,display:"flex",alignItems:"center",gap:8}}>
     Due This Month
     <span style={{background:"#fdf5e8",color:"#7A5A2A",fontSize:12,padding:"2px 10px",borderRadius:2,border:"1px solid #e8d8b0"}}>{dueThisMonth.length} invoice{dueThisMonth.length>1?"s":""} · ${dueThisMonth.reduce((s,inv)=>{const p=inv._type==="client"?pkgs.find(x=>x.id===inv.pkg):null;const items=inv._items||[{price:p?.price||0,qty:1}];const total=inv._type==="client"?(p?.price||0):items.reduce((ss,it)=>ss+it.qty*it.price,0);return s+Math.max(0,total-(inv.paid||0));},0).toLocaleString()} outstanding</span>
    </div>
    <Card sx={{overflow:"hidden"}}>
     <div style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr 1fr",padding:"7px 18px",borderBottom:"1px solid "+bo,background:"#fdfaf2"}}>
      {["Client","Session Date","Balance","Status"].map(h=><div key={h} style={{fontSize:12,letterSpacing:"0.1em",textTransform:"uppercase",color:br,fontFamily:D}}>{h}</div>)}
     </div>
     {dueThisMonth.map(inv=><InvRow key={inv.id} inv={inv} tint="#fffdf8"/>)}
    </Card>
   </div>}
  </div>;
 })()}
 <div style={{fontSize:13,letterSpacing:"0.15em",textTransform:"uppercase",color:br,fontFamily:D,fontWeight:600,marginBottom:8}}>All Invoices</div>
 <Card sx={{overflow:"hidden"}}>
 ...
 <div style={{display:"flex",gap:14,marginBottom:10}}>{[["",sun.rs],["",sun.ss]].map(([l,v])=><div key={l}><div style={{fontSize:9,color:tl,fontFamily:D}}>{l}</div><div style={{fontSize:11,fontWeight:600,color:es,fontFamily:D}}>{v}</div></div>)}</div>
 {c.loc&&<a href={"https://www.google.com/maps/dir/?api=1&destination="+encodeURIComponent(c.loc+(c.loc.toLowerCase().includes("fl")||c.loc.toLowerCase().includes("florida")?"":", Florida"))} target="_blank" rel="noreferrer"
  style={{display:"inline-block",background:pk,color:es,fontFamily:D,fontSize:13,padding:"6px 16px",borderRadius:2,textDecoration:"none",letterSpacing:"0.05em",border:"1px solid "+bo}}>Get Directions</a>}
 </Card>
 <WW shoot={wx}/>
 <Card sx={{padding:20}}>
 ...
 return<Card hover key={c.id} sx={{padding:14,cursor:"pointer",borderLeft:"3px solid "+(stageColors[stage]||bo)}} onClick={()=>{setSelC(c);setTab("people");}}>
 ...
 const prevMonth=()=>{const nm=new Date(yr,mo-1,1);setCalMonth(nm);fetchGcalMonth(nm.getFullYear(),nm.getMonth());};
 const nextMonth=()=>{const nm=new Date(yr,mo+1,1);setCalMonth(nm);fetchGcalMonth(nm.getFullYear(),nm.getMonth());};
 ...
 const gcalByDay=gcalMonthEvents[key]||{};
 const today=new Date();
 // Load this month on first render
 if(!gcalMonthEvents[key]&&!gcalMonthLoading){fetchGcalMonth(yr,mo);}
 return<div className="fi">
 <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:20}}>
 <h2 style={{fontFamily:D,fontSize:36,color:es,fontWeight:300}}>{monthName}</h2>
 <div style={{display:"flex",gap:8,alignItems:"center"}}>
 {gcalMonthLoading&&<div style={{fontFamily:D,fontSize:13,color:br}}>Syncing Google Calendar…</div>}
 {!gcalMonthLoading&&gcalConnected&&<div style={{fontFamily:D,fontSize:12,color:gn,background:gnl,padding:"4px 10px",borderRadius:2,border:"1px solid #c3dbc3"}}>Google Calendar synced</div>}
 {!gcalMonthLoading&&!gcalConnected&&<button onClick={()=>fetchGcalMonth(yr,mo)} style={{background:"transparent",border:"1px solid "+bo,borderRadius:2,padding:"6px 12px",fontFamily:D,fontSize:12,cursor:"pointer",color:br}}>Load Google Calendar</button>}
 ...
 <button onClick={()=>{const t=new Date();setCalMonth(new Date(t.getFullYear(),t.getMonth(),1));fetchGcalMonth(t.getFullYear(),t.getMonth());}} style={{background:"transparent",border:"1px solid "+bo,borderRadius:2,padding:"7px 14px",fontFamily:D,fontSize:13,cursor:"pointer",color:br,letterSpacing:"0.1em",textTransform:"uppercase"}}>Today</button>
 ...
 <div style={{display:"flex",alignItems:"center",gap:5}}><div style={{width:10,height:10,borderRadius:2,background:pk}}/><span style={{fontFamily:D,fontSize:12,color:br}}>CRM Session</span></div>
 <div style={{display:"flex",alignItems:"center",gap:5}}><div style={{width:10,height:10,borderRadius:2,background:"#e8f0fe"}}/><span style={{fontFamily:D,fontSize:12,color:br}}>Google Calendar</span></div>
 </div>
 ...
 {Array(firstDay).fill(null).map((_,i)=><div key={"e"+i} style={{minHeight:120}}/>)}
 {Array.from({length:daysInMonth},(_,i)=>i+1).map(d=>{
 const isToday=today.getFullYear()===yr&&today.getMonth()===mo&&today.getDate()===d;
 const daySessions=byDay[d]||[];
 const dayGcal=(gcalByDay[d]||[]).filter(e=>!e.isCRM);
 const hasAny=daySessions.length>0||dayGcal.length>0;
 return<div key={d} style={{minHeight:120,borderRadius:3,border:"1px solid "+(isToday?es:bo),background:isToday?"#fdf9f9":"#fff",padding:4,cursor:hasAny?"pointer":"default",transition:"background .1s"}}
 onClick={()=>{if(daySessions.length===1&&dayGcal.length===0){setSelC(daySessions[0]);setTab("people");}}}>
 <div style={{fontFamily:D,fontSize:15,marginBottom:2,width:22,height:22,borderRadius:"50%",background:isToday?es:"transparent",display:"flex",alignItems:"center",justifyContent:"center",color:isToday?"#fff":br,fontSize:12}}>{d}</div>
 {daySessions.map((c,ci)=>{const sun=getSun(c.date),p=pkgs.find(x=>x.id===c.pkg);return<div key={c.id} style={{background:pk,borderRadius:2,padding:"3px 7px",marginBottom:2,fontSize:11,fontFamily:D,color:es,lineHeight:1.4,overflow:"hidden"}}>
 <div style={{fontWeight:600,fontSize:12,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{c.first} {c.last}</div>
 <div style={{color:br,fontSize:9}}>{p?.name}</div>
 </div>;})}
 {dayGcal.slice(0,3).map((e,ei)=>{
 const startTime=e.start&&e.start.length>10?new Date(e.start).toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit"}):"";
 const isAllDay=e.start&&e.start.length===10;
 return<div key={e.id||ei} style={{background:"#e8f0fe",borderRadius:2,padding:"3px 7px",marginBottom:2,fontSize:11,fontFamily:D,color:"#3a5aa0",lineHeight:1.4,overflow:"hidden"}}>
 <div style={{fontSize:11,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{e.summary}</div>
 {!isAllDay&&startTime&&<div style={{fontSize:9,color:"#6080c0"}}>{startTime}</div>}
 </div>;})}
 {dayGcal.length>3&&<div style={{fontSize:9,color:br,fontFamily:D,paddingLeft:4}}>+{dayGcal.length-3} more</div>}
 </div>;
 })}
 </div>
 </Card>
 ...
 return<Card hover key={c.id} sx={{padding:12,cursor:"pointer"}} onClick={()=>{setSelC(c);setTab("people");}}>
 <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
 <div>
 ...
 {Object.keys(gcalByDay).length>0&&<div style={{marginTop:16}}>
 <div style={{fontSize:13,letterSpacing:"0.1em",textTransform:"uppercase",color:br,fontFamily:D,fontWeight:600,marginBottom:9}}>Other Google Events</div>
 <div style={{display:"flex",flexDirection:"column",gap:5}}>
 {Object.entries(gcalByDay).sort(([a],[b])=>Number(a)-Number(b)).flatMap(([day,evts])=>
  evts.filter(e=>!e.isCRM).map(e=>{
   const startTime=e.start&&e.start.length>10?new Date(e.start).toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit"}):"All day";
   return<div key={e.id||day+e.summary} style={{padding:"8px 12px",background:"#e8f0fe",borderRadius:2,border:"1px solid #c5d8f8"}}>
   <div style={{fontFamily:D,fontSize:14,color:"#3a5aa0"}}>{e.summary}</div>
   <div style={{fontFamily:D,fontSize:11,color:"#6080c0"}}>{new Date(yr,mo,Number(day)).toLocaleDateString("en-US",{weekday:"short",month:"short",day:"numeric"})} · {startTime}</div>
   </div>;
  })
 )}
 </div>
 ...
 <Btn sm v="o" ch="Duplicate" onClick={()=>setEditLocModal({...loc,id:Date.now().toString(),name:loc.name+" — Copy"})}/>
 ...
 {loc.notes&&<div style={{fontSize:12,color:tl,fontFamily:D,fontStyle:"italic",marginBottom:8}}>{loc.notes}</div>}
 {(loc.address||loc.name)&&<a href={"https://www.google.com/maps/dir/?api=1&destination="+encodeURIComponent((loc.address||loc.name)+", "+(loc.city||"Florida"))} target="_blank" rel="noreferrer"
  style={{display:"inline-block",background:"#fdf9f9",color:br,fontFamily:D,fontSize:12,padding:"4px 12px",borderRadius:2,textDecoration:"none",border:"1px solid "+bo,letterSpacing:"0.05em"}}>View on Map</a>}
 </Card>)}
 </div>
 ...
 {locations.find(l=>l.id===editLocModal.id)&&<Btn v="o" ch="Delete" onClick={async()=>{setLocations(ls=>ls.filter(l=>l.id!==editLocModal.id));await sb.from("locations").delete().eq("id",editLocModal.id);setEditLocModal(null);toast("Location removed.");}}/>}
 <Btn v="o" ch="Cancel" onClick={()=>setEditLocModal(null)}/>
 <Btn v="m" ch="Save Location" onClick={async()=>{
 if(locations.find(l=>l.id===editLocModal.id)){setLocations(ls=>ls.map(l=>l.id===editLocModal.id?editLocModal:l));await sb.from("locations").upsert(editLocModal);}
 else{setLocations(ls=>[...ls,editLocModal]);await sb.from("locations").insert(editLocModal);}
 ...
 <Btn sm v="m" ch="Save" onClick={async()=>{setQuestionnaires(qs=>qs.map(x=>x.id===q.id?q:x));await sb.from("questionnaires").upsert(q);toast("Questionnaire saved!");setSelQ(null);}}/>
 </div>
 <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:18}}>
 <Card sx={{padding:26}}>
 ...
 if(selBP){
  ...
  <div style={{display:"flex",gap:8}}>
  <Btn v="m" ch="Save" onClick={async()=>{setBookingProposalTypes(bps=>bps.map(b=>b.id===bp.id?bp:b));await sb.from("booking_proposal_types").upsert(bp);toast("Proposal type saved!");setSelBP(null);}}/>
 ...
 if(selThread){
  const t=selThread;
  const c=t.clientMatch;
  return<div className="fi">
  <div style={{display:"flex",gap:10,alignItems:"center",marginBottom:20}}>
   <button onClick={()=>setSelThread(null)} style={{background:"transparent",border:"none",cursor:"pointer",color:br,fontSize:17,fontFamily:D}}>← Back to Inbox</button>
   {c&&<div style={{marginLeft:"auto"}}><Btn sm v="o" ch={"Open "+c.first+"'s Profile"} onClick={()=>{setSelC(c);setTab("people");}}/></div>}
  </div>
  <div style={{display:"grid",gridTemplateColumns:"1fr 320px",gap:18}}>
   <div>
    <Card sx={{padding:24,marginBottom:14}}>
     <div style={{fontSize:13,letterSpacing:"0.12em",textTransform:"uppercase",color:br,fontFamily:D,marginBottom:4}}>From</div>
     <div style={{fontFamily:D,fontSize:18,color:es,marginBottom:2}}>{t.from}</div>
     <div style={{fontSize:14,color:br,fontFamily:D,marginBottom:12}}>{t.date}</div>
     <div style={{fontSize:14,letterSpacing:"0.12em",textTransform:"uppercase",color:br,fontFamily:D,marginBottom:6}}>Subject</div>
     <div style={{fontFamily:D,fontSize:20,color:es,marginBottom:20}}>{t.subject}</div>
     <div style={{fontFamily:D,fontSize:16,color:es,lineHeight:1.9,whiteSpace:"pre-wrap",background:"#fdf9f9",padding:"20px 22px",borderRadius:2,border:"1px solid "+bo}}>{t.body||t.snippet||"(no body)"}</div>
    </Card>
    <Card sx={{padding:24}}>
     <div style={{fontSize:13,letterSpacing:"0.12em",textTransform:"uppercase",color:br,fontFamily:D,marginBottom:10}}>Reply</div>
     <textarea value={replyDraft} onChange={e=>setReplyDraft(e.target.value)} rows={7} placeholder={"Hi "+((c?c.first:"")||"there")+",\n\n"} style={{width:"100%",border:"1px solid "+bo,borderRadius:2,padding:"14px 18px",fontSize:16,fontFamily:D,color:es,outline:"none",resize:"vertical",background:"#fff",lineHeight:1.8,marginBottom:12}}/>
     <div style={{display:"flex",gap:8}}>
      <Btn v="m" ch={replySending?"Sending…":"Send Reply"} onClick={()=>{if(replyDraft.trim())sendReply(t,replyDraft);}} sx={{opacity:replySending?0.7:1}}/>
      <Btn v="o" ch="AI Draft Reply" onClick={async()=>{
       if(!c)return toast("No client matched — AI needs a client profile");
       setAiModal(c);setAiDraft("");setAiPurpose("inquiry_response");
      }}/>
      <Btn v="o" ch="Cancel" onClick={()=>setSelThread(null)}/>
     </div>
    </Card>
   </div>
   {c&&<div>
    <Card sx={{padding:20,marginBottom:12}}>
     <div style={{fontSize:12,letterSpacing:"0.12em",textTransform:"uppercase",color:br,fontFamily:D,marginBottom:8}}>Matched Client</div>
     <div style={{fontFamily:D,fontSize:20,color:es,marginBottom:3}}>{c.first} {c.last}</div>
     <div style={{fontSize:13,color:br,fontFamily:D,marginBottom:10}}>{c.email}</div>
     <Tag color={SC(c.status)} ch={c.status}/>
     <div style={{marginTop:10,display:"grid",gridTemplateColumns:"1fr 1fr",gap:6}}>
      {[["Date",c.date||"TBD"],["Package",pkgs.find(p=>p.id===c.pkg)?.name||"TBD"],["Balance","$"+c.bal],["Location",c.loc||"TBD"]].map(([l,v])=><div key={l}><div style={{fontSize:11,color:tl,fontFamily:D,textTransform:"uppercase",marginBottom:1}}>{l}</div><div style={{fontFamily:D,fontSize:14,color:es,fontWeight:300}}>{v}</div></div>)}
     </div>
    </Card>
    {(c.emailLog||[]).length>0&&<Card sx={{padding:18}}>
     <div style={{fontSize:12,letterSpacing:"0.12em",textTransform:"uppercase",color:br,fontFamily:D,marginBottom:8}}>Email History</div>
     {(c.emailLog||[]).slice(-5).reverse().map((e,i)=><div key={i} style={{padding:"8px 0",borderBottom:i<(c.emailLog||[]).slice(-5).length-1?"1px solid "+bo:"none"}}>
      <div style={{display:"flex",justifyContent:"space-between",marginBottom:2}}>
       <div style={{fontSize:12,color:e.from==="michelle"?bl:es,fontFamily:D,fontWeight:500}}>{e.from==="michelle"?"You":"Client"}</div>
       <div style={{fontSize:11,color:tl,fontFamily:D}}>{new Date(e.date).toLocaleDateString()}</div>
      </div>
      <div style={{fontSize:13,color:br,fontFamily:D,lineHeight:1.5}}>{e.body?e.body.slice(0,80)+"…":e.subject}</div>
     </div>)}
    </Card>}
   </div>}
  </div>
 ...
 return<div className="fi">
 <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
  <h2 style={{fontFamily:D,fontSize:36,color:es,fontWeight:300}}>Inbox</h2>
  <div style={{display:"flex",gap:8}}>
   <Btn v="o" ch={inboxLoading?"Loading…":"Refresh Inbox"} onClick={fetchInbox} sx={{opacity:inboxLoading?0.7:1}}/>
  </div>
 </div>

 {reminders.length>0&&<div style={{marginBottom:18}}>
  <div style={{fontSize:13,letterSpacing:"0.12em",textTransform:"uppercase",color:rd,fontFamily:D,marginBottom:8}}>Invoice Reminders Needed</div>
  {reminders.map((r,i)=><div key={i} style={{display:"flex",alignItems:"center",gap:14,padding:"14px 18px",background:"#fdf0ee",border:"1px solid #e8c8c4",borderRadius:2,marginBottom:6}}>
   <div style={{flex:1}}>
    <div style={{fontFamily:D,fontSize:16,color:es}}>{r.client.first} {r.client.last}</div>
    <div style={{fontSize:13,color:rd,fontFamily:D}}>{r.label} — $${r.client.bal} due · Session {r.client.date}</div>
   </div>
   <Btn sm v="m" ch={"Send "+r.type+" Reminder"} onClick={()=>sendInvoiceReminder(r.client,r.type)}/>
   <Btn sm v="o" ch="View Client" onClick={()=>{setSelC(r.client);setTab("people");}}/>
  </div>)}
 </div>}

 {inboxThreads.length===0&&!inboxLoading&&<div>
  <div style={{marginBottom:18,padding:"16px 20px",background:"#fdf9f9",border:"1px solid "+bo,borderRadius:3}}>
   <div style={{fontFamily:D,fontSize:16,color:es,marginBottom:4}}>Gmail Inbox</div>
   <div style={{fontSize:14,color:br,fontFamily:D,lineHeight:1.8}}>Shows your last 30 days of client-related emails. Replies you send from here are automatically logged to the client record. Click Refresh Inbox to load messages from Gmail.</div>
  </div>
  {inboxQueue.length>0&&<div>
   <div style={{fontSize:13,letterSpacing:"0.12em",textTransform:"uppercase",color:br,fontFamily:D,marginBottom:10}}>Agent Review Queue</div>
   <Card sx={{overflow:"hidden"}}>
   {inboxQueue.map((item,i)=><div key={item.id} style={{display:"flex",alignItems:"center",gap:14,padding:"16px 20px",borderBottom:i<inboxQueue.length-1?"1px solid "+bo:"none"}}>
    <div style={{flex:1}}>
     <div style={{fontFamily:D,fontSize:17,color:es,marginBottom:2}}>{item.lead.first} {item.lead.last}</div>
 ...
 {inboxLoading&&<div style={{padding:"40px 0",textAlign:"center"}}><div className="pu" style={{fontFamily:D,fontSize:15,color:br}}>Loading inbox…</div></div>}

 {inboxThreads.length>0&&<Card sx={{overflow:"hidden"}}>
  <div style={{display:"grid",gridTemplateColumns:"160px 1fr 120px",padding:"10px 18px",borderBottom:"1px solid "+bo}}>
  {["From","Subject / Preview","Date"].map(h=><div key={h} style={{fontSize:12,letterSpacing:"0.1em",textTransform:"uppercase",color:br,fontFamily:D}}>{h}</div>)}
  </div>
  {inboxThreads.map((t,i)=><div key={t.id} className="rh" onClick={()=>setSelThread(t)} style={{display:"grid",gridTemplateColumns:"160px 1fr 120px",padding:"14px 18px",borderBottom:i<inboxThreads.length-1?"1px solid "+bo:"none",alignItems:"center",background:t.unread?"#fdfaf8":"transparent"}}>
   <div>
    <div style={{fontFamily:D,fontSize:15,color:es,fontWeight:t.unread?500:300,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{(t.from.match(/"?([^"<@]+)"?\s*</)?.[1]||t.from).trim().slice(0,22)}</div>
    {t.clientMatch&&<div style={{fontSize:11,color:gn,fontFamily:D,marginTop:1}}>Matched: {t.clientMatch.first} {t.clientMatch.last}</div>}
   </div>
   <div>
    <div style={{fontFamily:D,fontSize:15,color:es,marginBottom:2,fontWeight:t.unread?500:300}}>{t.subject}</div>
    <div style={{fontSize:12,color:br,fontFamily:D,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{t.snippet}</div>
   </div>
   <div style={{fontSize:13,color:br,fontFamily:D,textAlign:"right"}}>{t.date?new Date(t.date).toLocaleDateString("en-US",{month:"short",day:"numeric"}):"—"}</div>
  </div>)}
 </Card>}
 </div>;
 };

 const map={dashboard,calendar,invoices,scheduler,galleries,portal,settings};
 const renderTab=()=>{
 if(tab==="people")return selC?clientDetail():peopleTab();
