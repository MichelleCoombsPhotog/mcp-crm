...
Keep it to 4-6 sentences. Human, warm, NOT corporate. No lists. Sign as Michelle.`;
 ...
 const fetchClientMessages=async(c)=>{
  ...
  try{
   const data=await gmailCall("gmail_search_messages",{query:"from:"+c.email+" OR to:"+c.email,maxResults:40});
   const msgs=data&&(data.messages||data)||[];
   const threads=[];
   for(const msg of msgs.slice(0,25)){
    const detail=await gmailCall("gmail_read_message",{messageId:msg.id||msg.threadId});
    if(!detail)continue;
   ...
   // Merge with local emailLog
   const localLogs=(c.emailLog||[]).map(e=>({
   ...
   // Deduplicate by subject+date approximate match
   const all=[...threads,...localLogs.filter(l=>!threads.find(t=>t.subject===l.subject&&Math.abs(new Date(t.date)-new Date(l.date))<120000))];
   all.sort((a,b)=>new Date(b.date)-new Date(a.date));
   setClientMsgs(p=>({...p,[c.id]:all}));
  }catch(e){
   // Fall back to local log only
   const localLogs=(c.emailLog||[]).map(e=>({
    id:"local_"+Math.random(),
    subject:e.subject||"(sent from CRM)",
    from:e.from==="michelle"?"hello@michellecoombsphotography.com":c.email,
   ...
   setClientMsgs(p=>({...p,[c.id]:localLogs}));
  }
...
Keep it SHORT (5-8 sentences max). Sound like a real person, not a business. No bullet points in the email.`;
 ...
 <Btn v="o" ch="Open Inbox" onClick={()=>setTab("inbox")}/>
 ...
 {(()=>{
  ...
  const gmailMatches=inboxThreads.filter(t=>t.from.toLowerCase().includes(c.email.toLowerCase())||t.clientMatch?.id===c.id);
  const allMsgs=[
   ...log.map(e=>({...e,_src:"sent"})),
   ...gmailMatches.map(t=>({from:"client",body:t.body||t.snippet||"",date:t.date||"",subject:t.subject,_src:"gmail",_thread:t})),
  ].sort((a,b)=>{
   const da=new Date(a.date||0),db=new Date(b.date||0);
   return da-db;
  });
  const cMsg=clientMsgDraft,setCMsg=setClientMsgDraft;
  const cSub=clientMsgSub,setCSub=setClientMsgSub;
  const sendMsg=async()=>{
   if(!cMsg.trim())return;
   const resend=import.meta.env.VITE_RESEND_KEY;
   const entry={from:"michelle",body:cMsg,subject:cSub,date:new Date().toISOString()};
   if(resend){
    const res=await fetch("https://api.resend.com/emails",{method:"POST",headers:{"Content-Type":"application/json","Authorization":"Bearer "+resend},body:JSON.stringify({from:"Michelle Coombs <hello@michellecoombsphotography.com>",to:[c.email],subject:cSub,text:cMsg+"\n\nWith love,\nMichelle\nThe Collective | Michelle Coombs Photography",reply_to:"hello@michellecoombsphotography.com"})});
    if(res.ok){
     const upd={...c,emailLog:[...(c.emailLog||[]),entry]};
     setSelC(upd);setClients(cs=>cs.map(x=>x.id===c.id?upd:x));
     await sb.from("clients").update({emailLog:[...(c.emailLog||[]),entry]}).eq("id",c.id);
     setCMsg("");toast("Message sent to "+c.first+"!");
    }else toast("Send failed — check Resend key");
   }else{
    const upd={...c,emailLog:[...(c.emailLog||[]),entry]};
    setSelC(upd);setClients(cs=>cs.map(x=>x.id===c.id?upd:x));
    setCMsg("");toast("(Demo) Message logged — connect Resend to actually send");
   }
  };
  return<Card sx={{padding:22,marginTop:14}}>
   <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
    <div style={{fontSize:14,letterSpacing:"0.15em",textTransform:"uppercase",color:br,fontFamily:D}}>Messages</div>
    <div style={{display:"flex",gap:6,alignItems:"center"}}>
     {gmailMatches.length===0&&<Btn sm v="o" ch="Load from Gmail" onClick={fetchInbox}/>}
     <div style={{fontSize:12,color:tl,fontFamily:D}}>{allMsgs.length} message{allMsgs.length!==1?"s":""}</div>
    </div>
   </div>
   {allMsgs.length===0&&<div style={{fontFamily:D,fontSize:15,color:tl,marginBottom:16}}>No messages yet. Send one below.</div>}
   <div style={{maxHeight:340,overflowY:"auto",display:"flex",flexDirection:"column",gap:8,marginBottom:16}}>
   {allMsgs.map((m,i)=>{
    const fromMe=m.from==="michelle"||m._src==="sent";
    return<div key={i} style={{display:"flex",flexDirection:"column",alignItems:fromMe?"flex-end":"flex-start"}}>
     <div style={{maxWidth:"85%",background:fromMe?pk:"#f5f5f5",border:"1px solid "+(fromMe?bo:"#e0e0e0"),borderRadius:2,padding:"10px 14px"}}>
      {m.subject&&<div style={{fontSize:11,color:br,fontFamily:D,letterSpacing:"0.08em",textTransform:"uppercase",marginBottom:4}}>{m.subject}</div>}
      <div style={{fontFamily:D,fontSize:14,color:es,lineHeight:1.7,whiteSpace:"pre-wrap"}}>{(m.body||"").slice(0,400)}{(m.body||"").length>400?"…":""}</div>
     </div>
     <div style={{fontSize:11,color:tl,fontFamily:D,marginTop:2,paddingLeft:4,paddingRight:4}}>
      {fromMe?"You":"Client"} · {m.date?new Date(m.date).toLocaleDateString("en-US",{month:"short",day:"numeric",hour:"numeric",minute:"2-digit"}):"Unknown date"}
      {m._src==="gmail"&&<span style={{color:bl}}> (Gmail)</span>}
     </div>
     {m._src==="gmail"&&m._thread&&<button onClick={()=>setSelThread(m._thread)} style={{fontSize:11,color:bl,fontFamily:D,background:"none",border:"none",cursor:"pointer",paddingLeft:4,marginTop:1}}>Reply in Inbox →</button>}
    </div>;
   })}
   </div>
   <div style={{borderTop:"1px solid "+bo,paddingTop:14}}>
    <input value={cSub} onChange={e=>setCSub(e.target.value)} placeholder="Subject"
    ...
    <textarea value={cMsg} onChange={e=>setCMsg(e.target.value)} rows={4} placeholder={"Hi "+c.first+",\n\n"}
    style={{width:"100%",border:"1px solid "+bo,borderRadius:2,padding:"10px 14px",fontSize:14,fontFamily:D,color:es,outline:"none",resize:"vertical",background:"#fff",lineHeight:1.7,marginBottom:8}}/>
    <div style={{display:"flex",gap:7}}>
     <Btn v="m" ch="Send Message" onClick={sendMsg}/>
     <Btn v="o" ch="AI Draft" onClick={()=>{setAiModal(c);setAiDraft("");setAiPurpose("inquiry_response");}}/>
    </div>
   </div>
  </Card>;
 })()}
