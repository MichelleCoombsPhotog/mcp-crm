  ...
  const clientDetail=()=>{
    ...
    const FLD=({l,f,type="text",opts})=>{
      ...
      if(opts)return<div style={{padding:"8px 0",borderBottom:`1px solid ${bo}`}}>
        ...
        <select value={(draft[f]!==undefined?draft[f]:c[f])||""} onChange={e=>setEditDraft(d=>({...d,[f]:e.target.value}))} style={{width:"100%",border:`1px solid ${bo}`,borderRadius:2,padding:"8px 10px",fontSize:15,fontFamily:D,color:es,outline:"none",background:"#fff"}}>
          {opts.map(o=><option key={o}>{o}</option>)}
        </select>
      </div>;
      return<div style={{padding:"8px 0",borderBottom:`1px solid ${bo}`}}>
        <div style={{fontSize:11,letterSpacing:"0.15em",textTransform:"uppercase",color:br,fontFamily:D,marginBottom:4}}>{l}</div>
        <input type={type} value={(draft[f]!==undefined?draft[f]:c[f])||""} onChange={e=>setEditDraft(d=>({...d,[f]:e.target.value}))}
        ...
        <div>
          ...
          <Card sx={{padding:20,marginBottom:14}}>
            ...
            <div style={{marginTop:10}}>
              ...
              {editC?<textarea value={(draft.notes!==undefined?draft.notes:c.notes)||""} onChange={e=>setEditDraft(d=>({...d,notes:e.target.value}))} rows={3}
          ...
          <Card sx={{padding:20,marginBottom:14}}>
            ...
            {editC&&<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
              <div><div style={{fontSize:11,letterSpacing:"0.12em",textTransform:"uppercase",color:br,fontFamily:D,marginBottom:4}}>Paid</div>
                <input type="number" value={draft.paid!==undefined?draft.paid:c.paid} onChange={e=>setEditDraft(d=>({...d,paid:Number(e.target.value)}))} style={{width:"100%",border:`1px solid ${bo}`,borderRadius:2,padding:"8px 10px",fontSize:15,fontFamily:D,color:es,outline:"none",background:"#fff"}}/></div>
              <div><div style={{fontSize:11,letterSpacing:"0.12em",textTransform:"uppercase",color:br,fontFamily:D,marginBottom:4}}>Balance</div>
                <input type="number" value={draft.bal!==undefined?draft.bal:c.bal} onChange={e=>setEditDraft(d=>({...d,bal:Number(e.target.value)}))} style={{width:"100%",border:`1px solid ${bo}`,borderRadius:2,padding:"8px 10px",fontSize:15,fontFamily:D,color:es,outline:"none",background:"#fff"}}/></div>
            </div>}
          </Card>
          <Card sx={{padding:20}}>
