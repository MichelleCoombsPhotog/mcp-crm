...
Make it feel personal, reference their specific details naturally. Keep it warm, brief, and on-brand for Michelle.`;
 ...
 {[["Send Email","o"],["View Contract","o"]].map(([l,v])=><Btn key={l} v={v} ch={l} onClick={()=>toast(`${l} — coming soon!`)}/>)}
 </div>
 </Card>
 ...
 <Card sx={{padding:24}}>
 <div style={{fontFamily:D,fontSize:20,color:es,marginBottom:11}}>️ Beach Session Rules</div>
 ...
 <div style={{fontSize:13,letterSpacing:"0.1em",textTransform:"uppercase",color:br,fontFamily:D,fontWeight:600,marginBottom:5}}>Auto-detected beaches</div>
 <div style={{display:"flex",flexWrap:"wrap",gap:4,marginBottom:14}}>{BEACHES.map(b=><Tag key={b} color="blue" ch={b}/>)}</div>
 <div style={{fontFamily:D,fontSize:20,color:es,marginBottom:7}}> Parks / Studios</div>
 ...
 <Card sx={{padding:24}}>
 <div style={{fontFamily:D,fontSize:20,color:es,marginBottom:12}}> Live Preview</div>
 <div style={{marginBottom:9}}><div style={{fontSize:13,letterSpacing:"0.1em",textTransform:"uppercase",color:br,fontFamily:D,fontWeight:600,marginBottom:3}}>Date</div><input type="date" value={pvDate} onChange={e=>setPvDate(e.target.value)} style={{width:"100%",border:`1px solid ${bo}`,borderRadius:5,padding:"6px 9px",fontSize:12,fontFamily:U,outline:"none",background:cr}}/></div>
 <div style={{marginBottom:9}}><div style={{fontSize:13,letterSpacing:"0.1em",textTransform:"uppercase",color:br,fontFamily:D,fontWeight:600,marginBottom:3}}>Package</div><select value={pvPkg} onChange={e=>setPvPkg(e.target.value)} style={{width:"100%",border:`1px solid ${bo}`,borderRadius:5,padding:"6px 9px",fontSize:12,fontFamily:U,outline:"none",background:cr}}>{pkgs.map(p=><option key={p.id} value={p.id}>{p.name} ({p.dur}m)</option>)}</select></div>
 <div style={{marginBottom:11}}><div style={{fontSize:13,letterSpacing:"0.1em",textTransform:"uppercase",color:br,fontFamily:D,fontWeight:600,marginBottom:3}}>Location</div><select value={pvLoc} onChange={e=>setPvLoc(e.target.value)} style={{width:"100%",border:`1px solid ${bo}`,borderRadius:5,padding:"6px 9px",fontSize:12,fontFamily:U,outline:"none",background:cr}}>{["Cocoa Beach","New Smyrna Beach","Daytona Beach","Disney's BoardWalk","Orlando/Winter Park","Deltona Studio"].map(l=><option key={l}>{l}</option>)}</select></div>
 ...
 <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:18}}>
 <Card sx={{padding:24}}>
 <div style={{fontFamily:D,fontSize:20,color:es,marginBottom:3}}>Portrait Photography Agreement</div>
 ...
 <div>
 <div style={{fontSize:13,letterSpacing:"0.1em",textTransform:"uppercase",color:br,fontFamily:D,fontWeight:600,marginBottom:9}}>Signed Contracts</div>
 <Card sx={{overflow:"hidden",marginBottom:11}}>
 {clients.filter(c=>c.signed).map((c,i,arr)=><div key={c.id} style={{display:"flex",alignItems:"center",gap:11,padding:"15px 18px",borderBottom:i<arr.length-1?`1px solid ${bo}`:"none"}}>
 <div style={{flex:1}}><div style={{fontFamily:D,fontSize:19,color:es}}>{c.first} {c.last}</div><div style={{fontSize:12,color:br,fontFamily:D}}>{c.date} · {c.loc}</div></div>
 ...
 return<div key={d} style={{minHeight:110,borderRadius:3,border:"1px solid "+(isToday?es:bo),background:isToday?"#fdf9f9":"#fff",padding:4,cursor:hasSession?"pointer":"default",transition:"background .1s"}}
