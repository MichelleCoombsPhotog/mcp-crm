...
Make it feel personal, reference their specific details naturally. Keep it warm, brief, and on-brand for Michelle.`;
 ...
 const fetchGcalMonth=async(yr,mo)=>{
  ...
  const data=await gcalCall("gcal_list_events",{calendarId:"primary",timeMin,timeMax,maxResults:100});
  if(data){
...
Keep it SHORT (5-8 sentences max). Sound like a real person, not a business. No bullet points in the email.`;
 ...
 return<div key={d} style={{minHeight:90,borderRadius:3,border:"1px solid "+(isToday?es:bo),background:isToday?"#fdf9f9":"#fff",padding:4,cursor:hasAny?"pointer":"default",transition:"background .1s"}}
 onClick={()=>{if(daySessions.length===1&&dayGcal.length===0){setSelC(daySessions[0]);setTab("people");}}}>
 <div style={{fontFamily:D,fontSize:15,marginBottom:3,width:26,height:26,borderRadius:"50%",background:isToday?es:"transparent",display:"flex",alignItems:"center",justifyContent:"center",color:isToday?"#fff":br}}>{d}</div>
