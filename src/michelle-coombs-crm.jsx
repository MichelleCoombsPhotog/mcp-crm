...
const Tag=({color,ch})=>{const s=TK[color]||TK.gray;return<span style={{background:s.bg,color:s.tx,fontSize:10,fontFamily:U,fontWeight:600,letterSpacing:"0.1em",textTransform:"uppercase",padding:"3px 9px",borderRadius:20}}>{ch}</span>};
const Btn=({onClick,ch,v="p",sm,sx={}})=>{
  ...
  return<button onClick={onClick} className="bh" style={{background:s.bg,color:s.tx,border:s.border||"none",borderRadius:4,padding:sm?"5px 11px":"8px 18px",fontSize:sm?10:11,fontFamily:U,fontWeight:600,letterSpacing:"0.1em",textTransform:"uppercase",cursor:"pointer",transition:"all .18s",...sx}}>{ch}</button>
};
...
const Inp=({label,value,onChange,type="text",multi,rows=4,ph})=><div style={{marginBottom:11}}>
  ...
  :<input type={type} value={value} onChange={onChange} placeholder={ph} style={{width:"100%",border:`1px solid ${bo}`,borderRadius:6,padding:"7px 10px",fontSize:12,fontFamily:U,color:es,outline:"none",background:cr}}/>}
</div>;
const WW=({shoot})=>{
  ...
  return<Card hover sx={{padding:14,background:`linear-gradient(135deg,${shoot.pop>40?"#F9EDED":"#EDF2F8"},${wh})`}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:7}}>
      <div><div style={{fontSize:9,letterSpacing:"0.12em",textTransform:"uppercase",color:br,fontFamily:U,fontWeight:600,marginBottom:2}}>Shoot Date</div>
        <div style={{fontFamily:D,fontSize:13,color:es}}>{shoot.client}</div>
        <div style={{fontSize:10,color:br,fontFamily:U}}>{new Date(shoot.date+"T12:00:00").toLocaleDateString("en-US",{weekday:"short",month:"short",day:"numeric"})}</div>
      </div>
    ...
    <div style={{fontSize:10,color:br,fontFamily:U,marginBottom:7}}>{shoot.cond} · Low {shoot.lo}°</div>
...
export default function MCRM(){
  ...
  const sidebar=()=><div style={{width:open?205:54,minHeight:"100vh",background:es,flexShrink:0,display:"flex",flexDirection:"column",transition:"width .22s",overflow:"hidden"}}>
    <div style={{padding:open?"22px 18px 14px":"22px 10px 14px",borderBottom:"1px solid rgba(255,255,255,.07)",display:"flex",alignItems:"center",gap:9}}>
      ...
      {open&&<div><div style={{fontFamily:D,fontSize:12,color:cr,lineHeight:1.2}}>The Collective</div><div style={{fontSize:9,letterSpacing:"0.12em",color:ta,textTransform:"uppercase",fontFamily:U}}>Studio CRM</div></div>}
    </div>
    <nav style={{flex:1,padding:"10px 0",overflowY:"auto"}}>
      {NAV.map(n=><button key={n.id} className="nb" onClick={()=>{setTab(n.id);setSelC(null);setSelE(null);setSelP(null);}} style={{width:"100%",display:"flex",alignItems:"center",gap:9,padding:open?"8px 18px":"8px 14px",background:tab===n.id?"rgba(201,168,76,.14)":"transparent",borderLeft:`3px solid ${tab===n.id?go:"transparent"}`,border:"none",cursor:"pointer",transition:"all .14s",textAlign:"left"}}>
        <span style={{fontSize:13,flexShrink:0,width:15}}>{n.i}</span>
        {open&&<span style={{fontSize:11,fontFamily:U,fontWeight:tab===n.id?600:400,color:tab===n.id?go:ta,whiteSpace:"nowrap"}}>{n.l}</span>}
      </button>)}
    </nav>
  ...
  const dashboard=()=><div className="fi">
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:11,marginBottom:18}}>
      {[["💰","Revenue",`$${rev.toLocaleString()}`],["⏳","Outstanding",`$${out.toLocaleString()}`],["🌟","Active",clients.filter(c=>["Active","Booked","Proposal Sent"].includes(c.status)).length],["📥","New Leads",clients.filter(c=>c.status==="Lead").length]].map(([i,l,v])=><Card hover key={l} sx={{padding:15}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
          <div><div style={{fontSize:9,letterSpacing:"0.12em",textTransform:"uppercase",color:br,fontFamily:U,fontWeight:600,marginBottom:4}}>{l}</div><div style={{fontFamily:D,fontSize:22,color:es,lineHeight:1}}>{v}</div></div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1.6fr",gap:18,marginBottom:18}}>
      <div>
        <div style={{fontSize:9,letterSpacing:"0.12em",textTransform:"uppercase",color:br,fontFamily:U,fontWeight:600,marginBottom:9}}>☁️ Shoot Day Weather</div>
        <div style={{display:"flex",flexDirection:"column",gap:9}}>{WX.slice(0,2).map(w=><WW key={w.date} shoot={w}/>)}</div>
      </div>
      <div>
        <div style={{fontSize:9,letterSpacing:"0.12em",textTransform:"uppercase",color:br,fontFamily:U,fontWeight:600,marginBottom:9}}>📅 Upcoming Sessions</div>
        <Card sx={{overflow:"hidden"}}>
          {upcoming.slice(0,4).map((c,i,arr)=>{const sun=getSun(c.date),p=pkgs.find(x=>x.id===c.pkg);return<div key={c.id} className="rh" onClick={()=>{setSelC(c);setTab("clients");}} style={{display:"flex",alignItems:"center",gap:11,padding:"11px 14px",borderBottom:i<arr.length-1?`1px solid ${bo}`:"none",transition:"background .14s"}}>
            ...
            <div style={{flex:1}}><div style={{fontFamily:D,fontSize:14,color:es}}>{c.first} {c.last}</div><div style={{fontSize:10,color:br,fontFamily:U}}>{c.loc} · {p?.name}</div></div>
    ...
    <Card sx={{overflow:"hidden"}}>
      {clients.filter(c=>["Lead","Proposal Sent"].includes(c.status)).map((c,i,arr)=><div key={c.id} className="rh" onClick={()=>{setSelC(c);setTab("clients");}} style={{display:"flex",alignItems:"center",gap:12,padding:"11px 14px",borderBottom:i<arr.length-1?`1px solid ${bo}`:"none",transition:"background .14s"}}>
        <div style={{flex:1}}><div style={{fontFamily:D,fontSize:14,color:es}}>{c.first} {c.last}</div><div style={{fontSize:10,color:br,fontFamily:U}}>{c.type} · {c.date||"TBD"}</div></div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1.5fr 1fr",gap:18}}>
        <div>
          <Card sx={{padding:18,marginBottom:14}}>
            <div style={{fontSize:9,letterSpacing:"0.12em",textTransform:"uppercase",color:br,fontFamily:U,fontWeight:600,marginBottom:11}}>Session Details</div>
            ...
            {c.notes&&<div style={{marginTop:11,paddingTop:11,borderTop:`1px solid ${bo}`}}><div style={{fontSize:10,color:br,fontFamily:U,lineHeight:1.6}}>{c.notes}</div></div>}
          </Card>
          <Card sx={{padding:18,marginBottom:14}}>
            <div style={{fontSize:9,letterSpacing:"0.12em",textTransform:"uppercase",color:br,fontFamily:U,fontWeight:600,marginBottom:10}}>Financials</div>
          ...
          <Card sx={{padding:18}}>
            <div style={{fontSize:9,letterSpacing:"0.12em",textTransform:"uppercase",color:br,fontFamily:U,fontWeight:600,marginBottom:9}}>🖼️ Pic-Time Gallery</div>
        ...
        <div>
          ...
          {sun&&<Card sx={{padding:14,marginBottom:12,background:`linear-gradient(135deg,#FAF3E0,${wh})`}}>
            <div style={{fontSize:9,letterSpacing:"0.12em",textTransform:"uppercase",color:br,fontFamily:U,fontWeight:600,marginBottom:9}}>🌅 Ideal Times</div>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:7}}>
              <div><div style={{fontSize:9,color:tl,fontFamily:U}}>Sunrise</div><div style={{fontFamily:D,fontSize:15,color:es}}>{sun.rs}</div></div>
              <div><div style={{fontSize:9,color:tl,fontFamily:U}}>Sunset</div><div style={{fontFamily:D,fontSize:15,color:es}}>{sun.ss}</div></div>
            </div>
            {isB(c.loc)&&p&&<div style={{background:ml,borderRadius:7,padding:"7px 11px"}}>
              ...
              <div style={{fontSize:11,color:dk,fontFamily:U}}>🌅 Morning: {m2t(sun.rm-15)}</div>
              <div style={{fontSize:11,color:dk,fontFamily:U}}>🌇 Evening: {m2t(sun.sm-p.dur)}</div>
            </div>}
          </Card>}
          <Card sx={{padding:14}}>
            <div style={{fontSize:9,letterSpacing:"0.12em",textTransform:"uppercase",color:br,fontFamily:U,fontWeight:600,marginBottom:9}}>Quick Actions</div>
  ...
  const clientsList=()=><div className="fi">
    ...
    <Card sx={{overflow:"hidden"}}>
      <div style={{display:"grid",gridTemplateColumns:"2fr 1fr 1.2fr 1fr .8fr .8fr",padding:"7px 14px",borderBottom:`1px solid ${bo}`}}>
        {["Client","Type","Location","Date","Status","Balance"].map(h=><div key={h} style={{fontSize:9,letterSpacing:"0.12em",textTransform:"uppercase",color:br,fontFamily:U,fontWeight:600}}>{h}</div>)}
      </div>
      {clients.map((c,i)=><div key={c.id} className="rh" onClick={()=>setSelC(c)} style={{display:"grid",gridTemplateColumns:"2fr 1fr 1.2fr 1fr .8fr .8fr",padding:"12px 14px",borderBottom:i<clients.length-1?`1px solid ${bo}`:"none",alignItems:"center",transition:"background .14s"}}>
        <div><div style={{fontFamily:D,fontSize:14,color:es}}>{c.first} {c.last}</div><div style={{fontSize:10,color:tl,fontFamily:U}}>{c.email}</div></div>
        <div style={{fontSize:11,color:br,fontFamily:U}}>{c.type}</div>
        <div style={{fontSize:11,color:br,fontFamily:U}}>{c.loc}</div>
        <div style={{fontSize:11,color:br,fontFamily:U}}>{c.date||"TBD"}</div>
        <Tag color={SC(c.status)} ch={c.status}/>
    ...
    if(selE)return<div className="fi">
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:18}}>
        <Card sx={{padding:18}}>
          ...
          <div style={{fontSize:10,color:br,fontFamily:U,marginBottom:10,background:cr,padding:9,borderRadius:7,lineHeight:1.7}}>Merge tags: {"{first_name} {session_date} {session_time} {session_location}"}</div>
        ...
        <Card sx={{padding:18}}>
          <div style={{fontSize:9,letterSpacing:"0.12em",textTransform:"uppercase",color:br,fontFamily:U,fontWeight:600,marginBottom:10}}>Preview</div>
    ...
    return<div className="fi">
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:11}}>
        {emails.map(e=><Card hover key={e.id} sx={{padding:14,cursor:"pointer"}} onClick={()=>setSelE({...e})}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:5}}><div style={{fontFamily:D,fontSize:14,color:es,flex:1,marginRight:7}}>{e.name}</div><Btn sm v="o" ch="Edit" onClick={ev=>{ev.stopPropagation();setSelE({...e})}}/></div>
          <div style={{fontSize:10,color:br,fontFamily:U,marginBottom:2}}>📧 {e.subject}</div>
  ...
  const packagesTab=()=>{
    if(selP)return<div className="fi">
      ...
      <Card sx={{padding:22,maxWidth:480}}>
        ...
        <div style={{display:"flex",gap:7}}><Btn v="m" ch="Save" onClick={async()=>{setPkgs(ps=>ps.map(p=>p.id===selP.id?selP:p));await sb.from("packages").upsert({...selP,description:selP.desc});toast("Saved! ✓");setSelP(null)}}/><Btn v="o" ch="Cancel" onClick={()=>setSelP(null)}/></div>
      </Card>
    </div>;
    return<div className="fi">
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:11,marginBottom:22}}>
        {pkgs.map(p=><Card hover key={p.id} sx={{padding:18,position:"relative",overflow:"hidden"}}>
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:7}}><div><div style={{fontFamily:D,fontSize:16,color:es}}>{p.name}</div><div style={{fontFamily:D,fontSize:21,color:dk,marginTop:1}}>${p.price.toLocaleString()}</div></div><Btn sm v="o" ch="Edit" onClick={()=>setSelP({...p})}/></div>
          <div style={{fontSize:11,color:br,fontFamily:U,lineHeight:1.6,marginBottom:7}}>{p.desc}</div>
      ...
      <Card sx={{overflow:"hidden"}}>
        {addons.map((a,i)=><div key={a.id} style={{display:"flex",alignItems:"center",gap:11,padding:"11px 14px",borderBottom:i<addons.length-1?`1px solid ${bo}`:"none"}}>
          <div style={{flex:1}}><div style={{fontFamily:D,fontSize:13,color:es}}>{a.name}</div></div>
  ...
  const workflows=()=>{
    ...
    return<div className="fi">
      ...
      <Card sx={{padding:20,marginBottom:18}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:`repeat(${wf.ms.length},1fr)`,gap:14}}>
          {wf.ms.map((m,mi)=><div key={m}>
            <div style={{fontSize:9,letterSpacing:"0.12em",textTransform:"uppercase",color:br,fontFamily:U,fontWeight:600,marginBottom:7}}>{m}</div>
              ...
              <div style={{background:st.bg,borderRadius:6,padding:"7px 10px",borderLeft:`3px solid ${st.c}`}}>
                <div style={{display:"flex",gap:6,alignItems:"flex-start"}}>
                  ...
                  <div><div style={{fontSize:9,fontWeight:700,color:st.c,fontFamily:U,textTransform:"uppercase",letterSpacing:"0.07em"}}>{s.type}</div>
                    <div style={{fontSize:10,color:es,fontFamily:U}}>{s.a}</div>
      ...
      <Card sx={{overflow:"hidden"}}>
        {clients.filter(c=>!["Completed","Closed"].includes(c.status)).map((c,i,arr)=><div key={c.id} style={{display:"flex",alignItems:"center",gap:11,padding:"11px 14px",borderBottom:i<arr.length-1?`1px solid ${bo}`:"none"}}>
          <div style={{width:7,height:7,borderRadius:"50%",background:gn,flexShrink:0}} className="pu"/>
          <div style={{flex:1}}><div style={{fontFamily:D,fontSize:13,color:es}}>{c.first} {c.last}</div><div style={{fontSize:10,color:br,fontFamily:U}}>{[...LWF.steps,...SWF.steps].find(s=>s.id===c.wf)?.a||"Active"}</div></div>
          <Tag color={SC(c.status)} ch={c.status}/>
        </div>)}
      </Card>
    </div>
  ...
  const scheduler=()=>{
    ...
    return<div className="fi">
      ...
      <div style={{display:"grid",gridTemplateColumns:"1.2fr 1fr",gap:18}}>
        <Card sx={{padding:18}}>
          <div style={{fontFamily:D,fontSize:15,color:es,marginBottom:11}}>🏖️ Beach Session Rules</div>
          <div style={{background:ml,borderRadius:7,padding:11,marginBottom:11}}><div style={{fontSize:11,fontWeight:700,color:md,fontFamily:U,marginBottom:4}}>✅ Beach times auto-restricted to:</div><div style={{fontSize:11,color:dk,fontFamily:U,lineHeight:1.9}}>🌅 <strong>Morning:</strong> Sunrise − 15 mins<br/>🌇 <strong>Evening:</strong> Sunset − session length<br/>🚫 <strong>Midday blocked completely</strong></div></div>
          <div style={{fontSize:9,letterSpacing:"0.1em",textTransform:"uppercase",color:br,fontFamily:U,fontWeight:600,marginBottom:5}}>Auto-detected beaches</div>
          <div style={{display:"flex",flexWrap:"wrap",gap:4,marginBottom:14}}>{BEACHES.map(b=><Tag key={b} color="blue" ch={b}/>)}</div>
          <div style={{fontFamily:D,fontSize:15,color:es,marginBottom:7}}>🌳 Parks / Studios</div>
          <div style={{background:gnl,borderRadius:7,padding:11}}><div style={{fontSize:11,color:gn,fontFamily:U,lineHeight:1.9}}>✅ Full range: 6:30 AM – 8:30 PM<br/>✨ Golden hour highlighted as ideal<br/>📅 Max 1 session/day</div></div>
        </Card>
        <Card sx={{padding:18}}>
          <div style={{fontFamily:D,fontSize:15,color:es,marginBottom:12}}>🔮 Live Preview</div>
          <div style={{marginBottom:9}}><div style={{fontSize:9,letterSpacing:"0.1em",textTransform:"uppercase",color:br,fontFamily:U,fontWeight:600,marginBottom:3}}>Date</div><input type="date" value={pvDate} onChange={e=>setPvDate(e.target.value)} style={{width:"100%",border:`1px solid ${bo}`,borderRadius:5,padding:"6px 9px",fontSize:12,fontFamily:U,outline:"none",background:cr}}/></div>
          <div style={{marginBottom:9}}><div style={{fontSize:9,letterSpacing:"0.1em",textTransform:"uppercase",color:br,fontFamily:U,fontWeight:600,marginBottom:3}}>Package</div><select value={pvPkg} onChange={e=>setPvPkg(e.target.value)} style={{width:"100%",border:`1px solid ${bo}`,borderRadius:5,padding:"6px 9px",fontSize:12,fontFamily:U,outline:"none",background:cr}}>{pkgs.map(p=><option key={p.id} value={p.id}>{p.name} ({p.dur}m)</option>)}</select></div>
          <div style={{marginBottom:11}}><div style={{fontSize:9,letterSpacing:"0.1em",textTransform:"uppercase",color:br,fontFamily:U,fontWeight:600,marginBottom:3}}>Location</div><select value={pvLoc} onChange={e=>setPvLoc(e.target.value)} style={{width:"100%",border:`1px solid ${bo}`,borderRadius:5,padding:"6px 9px",fontSize:12,fontFamily:U,outline:"none",background:cr}}>{["Cocoa Beach","New Smyrna Beach","Daytona Beach","Disney's BoardWalk","Orlando/Winter Park","Deltona Studio"].map(l=><option key={l}>{l}</option>)}</select></div>
  ...
  const contracts=()=><div className="fi">
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:18}}>
      <Card sx={{padding:18}}>
        <div style={{fontFamily:D,fontSize:15,color:es,marginBottom:3}}>Portrait Photography Agreement</div>
        <div style={{fontSize:10,color:br,fontFamily:U,marginBottom:12}}>Edit freely — only affects future contracts</div>
      ...
      <div>
        <div style={{fontSize:9,letterSpacing:"0.1em",textTransform:"uppercase",color:br,fontFamily:U,fontWeight:600,marginBottom:9}}>Signed Contracts</div>
        <Card sx={{overflow:"hidden",marginBottom:11}}>
          {clients.filter(c=>c.signed).map((c,i,arr)=><div key={c.id} style={{display:"flex",alignItems:"center",gap:11,padding:"11px 14px",borderBottom:i<arr.length-1?`1px solid ${bo}`:"none"}}>
            <div style={{flex:1}}><div style={{fontFamily:D,fontSize:13,color:es}}>{c.first} {c.last}</div><div style={{fontSize:10,color:br,fontFamily:U}}>{c.date} · {c.loc}</div></div>
            <Tag color="green" ch="Signed ✓"/><Btn sm v="o" ch="View"/>
          </div>)}
        </Card>
        <Card sx={{padding:12,background:ml,border:`1px solid ${ma}`}}><div style={{fontSize:11,fontWeight:600,color:md,fontFamily:U,marginBottom:4}}>💡 Per-Client Amendments</div><div style={{fontSize:11,color:dk,fontFamily:U,lineHeight:1.6}}>Open a client record → Create Contract to generate a custom version. Clients cannot change anything without you.</div></Card>
      </div>
    </div>
  </div>;

  const invoices=()=><div className="fi">
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:11,marginBottom:16}}>
      {[["Total",`$${(rev+out).toLocaleString()}`],["Collected",`$${rev.toLocaleString()}`],["Outstanding",`$${out.toLocaleString()}`]].map(([l,v])=><Card key={l} sx={{padding:14}}><div style={{fontSize:9,letterSpacing:"0.1em",textTransform:"uppercase",color:br,fontFamily:U,fontWeight:600,marginBottom:4}}>{l}</div><div style={{fontFamily:D,fontSize:22,color:es}}>{v}</div></Card>)}
    </div>
    <Card sx={{overflow:"hidden"}}>
      {clients.filter(c=>c.paid>0||c.bal>0).map((c,i,arr)=>{const p=pkgs.find(x=>x.id===c.pkg);return<div key={c.id} className="rh" style={{display:"flex",alignItems:"center",gap:12,padding:"11px 14px",borderBottom:i<arr.length-1?`1px solid ${bo}`:"none"}}>
        <div style={{flex:1.5}}><div style={{fontFamily:D,fontSize:13,color:es}}>{c.first} {c.last}</div><div style={{fontSize:10,color:br,fontFamily:U}}>{p?.name}</div></div>
        <div style={{fontSize:11,color:br,fontFamily:U,flex:1}}>{c.date}</div>
        <div style={{fontFamily:D,fontSize:13,color:es,flex:0.8}}>${p?.price.toLocaleString()}</div>
  ...
  const galleries=()=><div className="fi">
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:11}}>
      {clients.map(c=><Card hover key={c.id} sx={{padding:14}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:7}}><div><div style={{fontFamily:D,fontSize:13,color:es}}>{c.first} {c.last}</div><div style={{fontSize:10,color:br,fontFamily:U}}>{c.date}</div></div><Tag color={c.gallery?"green":"gray"} ch={c.gallery?"Delivered":"Pending"}/></div>
  ...
  const portal=()=>{
    const c=clients[0],p=pkgs.find(x=>x.id===c.pkg),sun=getSun(c.date),wx=WX[0];
    return<div className="fi">
      <H2>Client Portal Preview</H2>
      <div style={{background:sa,borderRadius:9,padding:18}}>
        <div style={{background:wh,borderRadius:6,padding:"9px 18px",display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}><div style={{fontFamily:D,fontSize:13,color:es}}>The Collective · Michelle Coombs Photography</div><div style={{fontSize:11,color:br,fontFamily:U}}>Welcome, {c.first}!</div></div>
        <div style={{background:es,borderRadius:6,padding:"22px 18px",textAlign:"center",marginBottom:12}}><div style={{fontFamily:D,fontSize:22,color:cr,marginBottom:2}}>Hi {c.first}! 🎉</div><div style={{fontSize:12,color:ta,fontFamily:U}}>Your session is coming up — everything in one place.</div></div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:11}}>
          <Card sx={{padding:14}}>
            ...
            <div style={{fontFamily:D,fontSize:15,color:es,marginBottom:2}}>{new Date(c.date+"T12:00:00").toLocaleDateString("en-US",{weekday:"long",month:"long",day:"numeric"})}</div>
            <div style={{fontSize:11,color:br,fontFamily:U,marginBottom:9}}>{c.loc} · {p?.name}</div>
          ...
          <Card sx={{padding:14}}>
            ...
            {[["Package",`$${p?.price}`],["Retainer","-$150"],["Balance Due",`$${c.bal}`]].map(([l,v])=><div key={l} style={{display:"flex",justifyContent:"space-between",fontSize:12,fontFamily:U,marginBottom:3,color:l==="Balance Due"?rd:dk,fontWeight:l==="Balance Due"?700:400}}><span>{l}</span><span>{v}</span></div>)}
            {c.bal>0&&<Btn v="m" ch="Pay Balance Now" sx={{width:"100%",marginTop:7}}/>}
          </Card>
          <Card sx={{padding:14}}>
            ...
            <div style={{marginTop:7}}><Btn v="o" sm ch="Review"/></div>
          </Card>
        </div>
      </div>
    </div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:11}}>
      {[{n:"Square",d:"Payment processing",i:"💳"},{n:"Resend",d:"Email delivery (~3,000/mo free)",i:"✉️"},{n:"QuickBooks",d:"Sync invoices & financials",i:"📊"},{n:"Supabase",d:"Database — saves all data",i:"🗄️"},{n:"Zapier",d:"Connect Showit form → CRM",i:"⚡"},{n:"Pic-Time",d:"Gallery delivery via links",i:"🖼️",active:true}].map(s=><Card key={s.n} sx={{padding:14}}>
        <div style={{display:"flex",alignItems:"center",gap:9}}>
          ...
          <div style={{flex:1}}><div style={{fontFamily:D,fontSize:13,color:es,marginBottom:1}}>{s.n}</div><div style={{fontSize:10,color:br,fontFamily:U,marginBottom:4}}>{s.d}</div><Tag color={s.active?"green":"gray"} ch={s.active?"Active":"Not connected"}/></div>
  ...
  const leads=()=><div className="fi">
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:11}}>
      {["Lead","Proposal Sent","Booked","Completed"].map(stage=><div key={stage}>
        <div style={{fontSize:9,letterSpacing:"0.12em",textTransform:"uppercase",color:br,fontFamily:U,fontWeight:600,marginBottom:7,display:"flex",justifyContent:"space-between"}}>{stage}<span style={{color:tl}}>{clients.filter(c=>c.status===stage).length}</span></div>
        <div style={{display:"flex",flexDirection:"column",gap:7}}>
          {clients.filter(c=>c.status===stage).map(c=><Card hover key={c.id} sx={{padding:11,cursor:"pointer"}} onClick={()=>{setSelC(c);setTab("clients")}}>
            <div style={{fontFamily:D,fontSize:13,color:es,marginBottom:2}}>{c.first} {c.last}</div>
            <div style={{fontSize:10,color:br,fontFamily:U,marginBottom:2}}>{c.loc}</div>
  ...
  const calendar=()=>{
    ...
    return<div className="fi">
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 220px",gap:18}}>
        ...
        <div>
          <div style={{fontSize:9,letterSpacing:"0.1em",textTransform:"uppercase",color:br,fontFamily:U,fontWeight:600,marginBottom:7}}>Sessions</div>
          <div style={{display:"flex",flexDirection:"column",gap:7}}>
            {upcoming.map(c=>{const wx=WX.find(w=>w.date===c.date);return<Card key={c.id} sx={{padding:11}}>
              <div style={{display:"flex",justifyContent:"space-between"}}><div><div style={{fontFamily:D,fontSize:13,color:es}}>{c.first} {c.last}</div><div style={{fontSize:10,color:br,fontFamily:U}}>{c.date}</div></div>{wx&&<span style={{fontSize:16}}>{wx.icon}</span>}</div>
            </Card>})}
          </div>
        </div>
      </div>
    ...
    <div style={{background:wh,borderRadius:13,padding:24,width:"100%",maxWidth:460,boxShadow:"0 20px 50px rgba(44,24,16,.2)"}}>
      ...
      <div style={{display:"flex",gap:11}}><div style={{flex:1}}><Inp label="Session Date" value={nc.date} onChange={e=>setNc({...nc,date:e.target.value})} type="date"/></div>
        <div style={{flex:1}}><div style={{fontSize:10,letterSpacing:"0.1em",textTransform:"uppercase",color:br,fontFamily:U,fontWeight:600,marginBottom:3}}>Package</div>
          <select value={nc.pkg} onChange={e=>setNc({...nc,pkg:e.target.value})} style={{width:"100%",border:`1px solid ${bo}`,borderRadius:5,padding:"7px 9px",fontSize:12,fontFamily:U,color:es,outline:"none",background:cr}}>{pkgs.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select>
        </div>
      </div>
    ...
    <div style={{display:"flex",minHeight:"100vh",background:cr,fontFamily:U}}>
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
        <div style={{background:wh,borderBottom:`1px solid ${bo}`,padding:"10px 22px",display:"flex",justifyContent:"space-between",alignItems:"center",flexShrink:0}}>
          <div style={{fontFamily:D,fontSize:15,color:es,fontWeight:400}}>{NAV.find(n=>n.id===tab)?.i} {NAV.find(n=>n.id===tab)?.l}</div>
          <div style={{display:"flex",alignItems:"center",gap:11}}>
            <div style={{fontSize:11,color:br,fontFamily:U}}>📍 Deltona, FL</div>
